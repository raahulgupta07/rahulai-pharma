<script lang="ts">
 ...
 // slug = project slug; embedded = compact dashboard card; height = css height when embedded
 let { slug = '', embedded = false, height = '460px' } = $props<{
 slug?: string; embedded?: boolean; height?: string;
 }>();
 ...
 detail = null; hub = null; detailLoading = true; clinicalOpen = false;
 try {
 ...
 // hub = generic | category | indication aggregate detail
 async function loadHub(htype: string, name: string) {
 hub = null; detail = null; hubLoading = true;
 try {
 const r = await fetch(`/api/projects/${slug}/graph/hub?type=${htype}&id=${encodeURIComponent(name)}`, { headers: _h() });
 if (r.ok) hub = await r.json();
 } catch {} finally { hubLoading = false; }
 }
 function closeDetail() { selected = null; detail = null; hub = null; }
 ...
 selected = { id: e.node, name: attrs.label, ...attrs, degree: graph.degree(e.node) };
 // node ids are type-prefixed (b:/g:/c:/i:) — endpoints need the bare label.
 if (source === 'pharma') {
 if (attrs.group === 'Brand') loadDetail(attrs.label);
 else loadHub((attrs.group || '').toLowerCase(), attrs.label); // generic|category|indication
 }
 });
 }
 ...
 // open a drug profile from a hub member list (switch panel to that brand)
 function pickBrand(brand: string) {
 const nid = 'b:' + brand;
 if (graph && graph.hasNode(nid)) {
 selected = { id: nid, name: brand, ...graph.getNodeAttributes(nid), degree: graph.degree(nid) };
 } else {
 selected = { id: nid, name: brand, label: brand, group: 'Brand', color: '#6b6f82', degree: 0 };
 }
 loadDetail(brand);
 }
 function onKey(e: KeyboardEvent) { if (e.key === 'Escape') closeDetail(); }
 function openExplorer() { goto(`${base}/project/${slug}/graph`); }
...
<div class="gv-root" class:embedded style={embedded ? `height:${height}` : ''}>
  <!-- toolbar -->
  <div class="gv-bar">
    {#if embedded}
      <button class="gv-back" onclick={openExplorer}>[] Full explorer</button>
    {:else}
      <button class="gv-back" onclick={() => goto(`${base}/project/${slug}/overview`)}><Icon name="arrow-left" size={16} /> Dashboard</button>
    {/if}
    <div class="gv-seg">
    ...
    {#if source === 'pharma'}
      <div class="gv-search">
        ...
        {#if focus}<button class="gv-clear" onclick={clearFocus}><Icon name="x" size={16} /> {focus}</button>{/if}
      </div>
    {/if}
    ...
    {#if selected}
      <aside class="gv-detail">
        <header class="gd-head">
          ...
          <h3>{selected.label || hub?.title || '—'}</h3>
          <button class="gd-x" onclick={closeDetail} aria-label="close"><Icon name="x" size={16} /></button>
        </header>
        ...
        {#if source !== 'pharma'}
          ...
          <section class="gd-sec">
            <div class="gd-row"><label>type</label><span>{selected.group || '—'}</span></div>
            <div class="gd-row"><label>links</label><span>{selected.degree}</span></div>
          </section>
        {:else if selected.group !== 'Brand'}
          <!-- pharma HUB: generic / category / indication aggregate -->
          {#if hubLoading}
            <div class="gd-load">rolling up members…</div>
          {:else if hub}
            <section class="gd-sec">
              <h4>Overview</h4>
              {#if hub.overview.molecules != null}<div class="gd-row"><label>molecules</label><span>{fmt(hub.overview.molecules)}</span></div>{/if}
              <div class="gd-row"><label>brands</label><span>{fmt(hub.overview.brands)}</span></div>
              {#if hub.overview.categories != null}<div class="gd-row"><label>categories</label><span>{fmt(hub.overview.categories)}</span></div>{/if}
              {#if hub.overview.stock}
                <div class="gd-stockline"><b>{fmt(hub.overview.stock)}</b> units · {fmt(hub.overview.stores)} stores</div>
              {:else}
                <div class="gd-empty">no stock records</div>
              {/if}
              {#if hub.overview.avg_cost}<div class="gd-row"><label>avg cost</label><span>{fmt(hub.overview.avg_cost)} MMK</span></div>{/if}
            </section>

            {#if hub.clinical && (hub.clinical.indication || hub.clinical.composition)}
              <section class="gd-sec">
                <h4>Clinical (shared)</h4>
                {#if hub.clinical.composition}<div class="gd-row"><label>compose</label><span>{hub.clinical.composition}</span></div>{/if}
                {#if hub.clinical.indication}<div class="gd-row"><label>indication</label><span>{hub.clinical.indication}</span></div>{/if}
              </section>
            {/if}

            {#if hub.categories?.length}
              <section class="gd-sec">
                <h4>Spans categories ({hub.categories.length})</h4>
                <div class="gd-chips">{#each hub.categories as c}<span class="gd-cchip">{c}</span>{/each}</div>
              </section>
            {/if}

            {#if hub.molecules?.length}
              <section class="gd-sec">
                <h4>Molecules ({hub.molecules.length})</h4>
                <div class="gd-chips">{#each hub.molecules as m}<span class="gd-cchip">{m}</span>{/each}</div>
              </section>
            {/if}

            {#if hub.members?.length}
              <section class="gd-sec">
                <h4>{selected.group === 'Generic' ? 'Brands' : 'Drugs'} ({hub.members.length}{hub.members.length >= 30 ? '+' : ''}, top by stock)</h4>
                {#each hub.members as m}
                  <button class="gd-sub" onclick={() => pickBrand(m.brand)}>
                    <span class="gd-arrow"><Icon name="arrow-right" size={16} /></span>
                    <span class="gd-subname">{m.brand}</span>
                    {#if m.qty !== null && m.qty !== undefined}
                      <span class="gd-badge" class:out={m.qty === 0}>{m.qty === 0 ? '○' : '▣'} {fmt(m.qty)}</span>
                    {/if}
                  </button>
                {/each}
              </section>
            {/if}
          {:else}
            <div class="gd-empty" style="padding:12px 16px">no detail</div>
          {/if}
          ...
          {#if detail.clinical.indication || detail.clinical.dosage || detail.clinical.side_effect}
            <section class="gd-sec">
              ...
              {#if clinicalOpen}
                ...
                {#if detail.clinical.side_effect}<div class="gd-row gd-warn"><label><Icon name="alert-triangle" size={16} /> side fx</label><span>{detail.clinical.side_effect}</span></div>{/if}
              {/if}
            </section>
          {/if}
        ...
        {#if source === 'pharma' && selected.group === 'Brand'}
          <div class="gd-acts">
            <button class="gd-focus" onclick={() => egoFocus(selected.name)}>Focus 2-hop</button>
          </div>
        {/if}
      </aside>
    {/if}
...
<style>
 ...
 .gd-cchip { font-size: 10.5px; color: #cdbfe0; background: #251f30; border: 1px solid #2a2533; padding: 3px 8px; border-radius: 10px; line-height: 1.3; }
