...
<div class="ep-root">
  ...
  <div class="ep-actions">
    {#if status === 'running' || status === 'queued'}
      <button class="btn-cancel" type="button" onclick={onCancel}><Icon name="x" size={16} /> CANCEL</button>
    {/if}
    {#if status === 'done' || status === 'failed' || status === 'cancelled'}
      <button class="btn-rerun" type="button" onclick={onReRun}><Icon name="refresh" size={16} /> RE-RUN</button>
    {/if}
  </div>
</div>
