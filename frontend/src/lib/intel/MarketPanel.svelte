      ...
      <ul class="mp-sr-list">
        {#each searchResults as r}
          <li>
            ...
            {#if r.source_url}<a href={r.source_url} target="_blank" rel="noopener" class="mp-link"><Icon name="trending-up" size={16} /></a>{/if}
          </li>
        {/each}
      </ul>
        ...
        <ul class="mp-signal-list">
          {#each signals as s}
            <li>
              ...
              {#if s.source_url}<a href={s.source_url} target="_blank" rel="noopener" class="mp-link"><Icon name="trending-up" size={16} /></a>{/if}
            </li>
          {/each}
        </ul>
      {:else}
