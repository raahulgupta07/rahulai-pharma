...
<div class="cv-page">
  ...
  {#if err}<div class="err"><Icon name="alert-triangle" size={16} /> {err}</div>{/if}
    ...
    <div class="grid">
      {#each canvases as c (c.id)}
        <div class="card" role="button" tabindex="0"
          ...
          <button class="del" onclick={(e) => { e.stopPropagation(); del(c.id, c.name); }} title="Delete canvas"><Icon name="x" size={16} /></button>
        </div>
      {/each}
    </div>
  {/if}
