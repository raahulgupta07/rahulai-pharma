<script>
 ...
 // Verified glyph — show OK when a pinned-truth check passed.
...
<article class="answer-card">
  ...
  {#if mono}
    ...
    <div class="mono-head">
      <span class="mono-icon" aria-hidden="true"><Icon name="flask" size={16} /></span>
    ...
    {#if bandLead}
      <p class="band-lead"><span class="lead-arrow" aria-hidden="true"><Icon name="arrow-right" size={16} /></span> {@html formatInline(bandLead)}</p>
    {/if}
    ...
    {#if mono.caution.length || mono.interacts.length}
      <div class="mono-safety">
        {#each mono.caution as c}
          <div class="mono-safety-row"><span class="mono-safety-tag"><Icon name="alert-triangle" size={16} /> CAUTION</span><span class="mono-safety-text">{c}</span></div>
        {/each}
        {#each mono.interacts as it}
          <div class="mono-safety-row"><span class="mono-safety-tag"><Icon name="alert-triangle" size={16} /> INTERACTS</span><span class="mono-safety-text">{it}</span></div>
        {/each}
      </div>
    {/if}
    ...
    {#if mono.equiv.length}
      <hr />
      <div class="block-label"><Icon name="refresh" size={16} /> Therapeutic equivalents</div>
    ...
    {#if mono.evidence}
      <div class="mono-evidence"><Icon name="link" size={16} /> evidence&nbsp;&nbsp;{mono.evidence.article}{#if mono.evidence.table} · {mono.evidence.table}{/if}<span class="mono-verified"><Icon name="check" size={16} /> verified</span></div>
    {/if}
  {:else}
  ...
  {:else if actionTitle}
    <h2 class="action-title">
      <span class="title-icon" aria-hidden="true"><Icon name="flask" size={16} /></span>
  ...
  <!-- WHY / Show SQL block removed — the SQL is already visible inside the
       collapsed thought-trace steps (foldable code). SOURCES chips cite the
       datasets. Keeps the answer Aria-clean. -->
  ...
  {#if showBand && evidenceParts.length}
    <div class="evidence-row" title="Grounding">
      ...
      {/each}
      {#if isVerified}<span class="ev-verified">OK verified</span>{/if}
    </div>
  {/if}
...
<style>
 /* Claude/Aria-style: no card chrome — answer flows on the page. */
 .answer-card {
 position: relative;
 ...
 /* answer prose body — serif, airy, no box (Claude/Aria) */
 .summary-card {
 background: transparent;
 border-radius: 0;
 padding: 0;
