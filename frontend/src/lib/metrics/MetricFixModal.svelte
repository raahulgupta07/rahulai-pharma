      ...
      {#if testResult}
        ...
        ">
          <div style="font-weight:900; margin-bottom:4px;">
            {testResult.ok ? 'OK PASS' : 'x FAIL'} — total: {testResult.total ?? '—'}
          </div>
          {#if specVerifiedAns && testResult.total != null}
            <div style="font-size:10px;">
              ...
              {#if Number(testResult.total) === Number(specVerifiedAns)}
                <strong style="color:#2e7d32;"> OK MATCH</strong>
              {:else}
                <strong style="color:#c62828;"> x MISMATCH</strong>
              {/if}
            </div>
          {/if}
