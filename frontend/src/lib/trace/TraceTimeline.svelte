<script lang="ts">
 ...
 /** stable index key, assigned after the list is built (for per-row code fold) */
 _k?: number;
 name?: string;
 label: string;
 narrative?: string;
 ...
 // Dedupe back-to-back reasoning steps with identical prose — the model
 // often re-emits the same capability/plan blurb between tool calls, which
 // showed up as "Reasoning … / Reasoning …" twice. Keep the first only.
 const narr = typeof a.text === 'string' ? a.text.trim() : '';
 const prev = out[out.length - 1];
 if (narr && prev && prev.kind === 'step' && (prev.narrative || '').trim() === narr) {
 continue;
 }
 out.push({
 ...
 // Per-row code-block fold. Default: open in classic (detail view), folded in
 // the quiet claude variant. `codeOverride[_k]` records an explicit user click.
 let codeOverride = $state<Record<number, boolean>>({});
 function codeOpen(k: number | undefined): boolean {
 if (k == null) return variant !== 'claude';
 return codeOverride[k] ?? (variant !== 'claude');
 }
 function toggleCode(k: number | undefined) {
 if (k == null) return;
 codeOverride = { ...codeOverride, [k]: !codeOpen(k) };
 }

 // Present→past tense flip: a running step reads "Searching…", the same step
 // once done reads "Searched". Maps the leading gerund verb to its past form.
 const PAST: Record<string, string> = {
 Searching: 'Searched', Querying: 'Queried', Planning: 'Planned',
 Inspecting: 'Inspected', Building: 'Built', Running: 'Ran',
 Checking: 'Checked', Composing: 'Composed', Routing: 'Routed',
 Delegating: 'Delegated', Reading: 'Read', Writing: 'Wrote',
 Analyzing: 'Analyzed', Generating: 'Generated', Loading: 'Loaded',
 Listing: 'Listed',
 };
 function toPast(label: string): string {
 if (!label) return label;
 const m = label.match(/^([A-Z][a-z]+)\b/);
 if (m && PAST[m[1]]) return PAST[m[1]] + label.slice(m[1].length);
 return label;
 }

 // One-line gist for the collapsed header — the Claude-style summary. Built
 // from the distinct past-tense actions the run took (first 3), e.g.
 // "Planned with analyst, searched internal knowledge, queried the database".
 const gist = $derived.by(() => {
 const seen = new Set<string>();
 const phrases: string[] = [];
 for (const r of rows) {
 if (r.kind === 'route' || r.kind === 'summarize') continue;
 let t = toPast(title(r)).replace(/…$/, '').trim();
 if (!t || t === 'Reasoning') continue;
 const key = t.toLowerCase();
 if (seen.has(key)) continue;
 seen.add(key);
 phrases.push(phrases.length ? t.charAt(0).toLowerCase() + t.slice(1) : t);
 ...
 if (name === 'list_tables') return 'Listing tables';
 ...
 .replace(/^\s*[-–—•*]\s*$/gm, '') // empty bullet / dash placeholder lines
 .replace(/^\s*Expected output should include:\s*$/gim, '') // dangling label after empties
 ...
 parts.push(`^${fmtTokens(r.tokensIn || 0)} v${fmtTokens(r.tokensOut || 0)}`);
...
{#snippet rowItem(r: Row)}
  <li class="tt-row" class:tt-run={r.status === 'running'} class:tt-err={r.status === 'error'}>
    ...
    <div class="tt-content">
      <div class="tt-name">{r.status === 'done' ? toPast(title(r)) : title(r)}{#if r.kind === 'tool' && r.name}<span class="tt-tool-tag">{r.name}</span>{/if}</div>
      {#if r.kind === 'tool' && Array.isArray(r.autoFix) && r.autoFix.length}
        <div class="tt-autofix" title={r.autoFix.join(' · ')}>
          <span class="tt-autofix-icon"><Icon name="sparkles" size={16} /></span>
      ...
      {/if}
      {#if hasCode(r)}
        <button class="tt-codetog" class:open={codeOpen(r._k)} onclick={() => toggleCode(r._k)} type="button">
          <span class="tt-codetog-cv">▸</span>
          <span>{codeOpen(r._k) ? 'Hide' : 'View'} {sqlFromArgs(r.args) ? 'SQL' : 'call'}</span>
        </button>
        {#if codeOpen(r._k)}
          <div class="tt-code">
      ...
      <div class="tt-resline">
        ...
        {:else if resultSummary(r)}
          <span class="tt-result"><Icon name="arrow-right" size={16} /> {resultSummary(r)}</span>
        {/if}
...
{#if rows.length}
  <div class="tt" class:tt-live={live} class:tt-claude={variant === 'claude'}>
    {#if variant === 'claude'}
      <!-- Claude.ai-style: boxless gray summary line + trailing › chevron -->
      <button class="tt-head tt-line" class:tt-line-live={live} onclick={toggle} type="button" aria-expanded={!collapsed}>
        <span class="tt-title">{
          live
            ? (stepCount > 1 ? 'Working…' : 'Thinking…')
            : (gist || 'Thought process')
        }</span>
        <span class="tt-chev tt-chev-ico" class:open={!collapsed}><Icon name="chevron-right" size={14} /></span>
      </button>
    {:else}
      <button class="tt-head" onclick={toggle} type="button" aria-expanded={!collapsed}>
        ...
        {#if sqlAutoFixes.length}<span class="tt-chip tt-chip-autofix" title={sqlAutoFixes.join(' · ')}><Icon name="sparkles" size={16} /> {sqlAutoFixes.length} auto-fix{sqlAutoFixes.length === 1 ? '' : 'es'}</span>{/if}
        <span class="tt-stat">
        ...
        <span class="tt-chev" class:open={!collapsed}>v</span>
      </button>
    {/if}
...
<style>
 ...
 .tt-chev-ico { display: inline-flex; align-items: center; color: var(--pw-muted, #8a8175); }
 ...
 /* foldable code toggle — quiet "View SQL / Hide call" affordance */
 .tt-codetog { display: inline-flex; align-items: center; gap: 5px; margin: 7px 0 0; padding: 0; background: none; border: none; cursor: pointer; font: inherit; font-size: 11.5px; color: var(--pw-dim, #97968f); }
 .tt-codetog:hover { color: var(--pw-accent, #c96342); }
 .tt-codetog-cv { font-size: 9px; transition: transform 0.15s ease; }
 .tt-codetog.open .tt-codetog-cv { transform: rotate(90deg); }
 ...
 /* Claude.ai boxless summary line: gray text + trailing › chevron, no box */
 .tt-claude .tt-line {
 width: auto;
 display: inline-flex; align-items: center; gap: 6px;
 border: none; background: transparent; padding: 0;
 }
 .tt-claude .tt-line-live .tt-title { animation: tt-pulse 1.4s ease-in-out infinite; }
 .tt-claude .tt-chev-ico { color: var(--pw-dim, #b0a89c); }
 /* chevron-right → rotates to point down when expanded */
 .tt-claude .tt-line .tt-chev.open { transform: rotate(90deg); }
 .tt-claude .tt-head { color: var(--pw-muted, #8a8175); gap: 6px; padding: 3px 0; }
 .tt-claude .tt-title { font-weight: 400; font-size: 14px; color: var(--pw-muted, #8a8175); }
 .tt-claude .tt-line:hover .tt-title { color: var(--pw-ink, #2c2a26); }
 ...
 /* Claude shows humanized step titles only — hide raw function-name tags */
 .tt-claude .tt-tool-tag { display: none; }
