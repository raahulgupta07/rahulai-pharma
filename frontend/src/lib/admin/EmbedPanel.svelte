<script lang="ts">
 ...
 { group: 'MANAGE', items: [
 { id: 'brand', label: 'Global Settings', icon: 'sliders' },
...
{:else if !isSuper}
  <div class="emp-center">
    <div class="emp-denied">
      <div class="emp-denied-mark"><Icon name="x" size={16} /></div>
      ...
      {#if !embedded}
        <button class="emp-btn" onclick={() => goto('/ui/home')}><Icon name="arrow-left" size={16} /> back</button>
      {/if}
    </div>
  </div>
{:else}
  <div class="emp-wrap" class:emp-embedded={embedded}>
    ...
    <main class="emp-main">
      ...
      {#if view === 'overview'}
        ...
        <section class="emp-panel">
          ...
          <div class="emp-codeblock">
            <button class="emp-copybtn" onclick={() => copyText(widgetJsTest, 'wjstest')}>{copied === 'wjstest' ? 'OK' : 'copy'}</button>
        ...
        <section class="emp-panel">
          ...
          <div class="emp-statusbar">
            ...
            <button class="emp-btn emp-btn-sm" onclick={() => nav('brand')}>Edit Brand <Icon name="arrow-right" size={16} /></button>
          </div>
      ...
      {#if view === 'brand'}
        <div class="emp-pg-grid">
          <section class="emp-panel emp-pg-col">
            <div class="emp-h"><Icon name="zap" size={14} /> DEFAULT MODEL</div>
            <p class="emp-doc-p">The engine every widget uses unless a store overrides it.</p>
            <div class="emp-config-grid">
              <label class="emp-field emp-field-full">
                <span class="emp-flabel">model for all widgets</span>
                <div class="emp-color-row">
                  <select class="emp-input emp-select" bind:value={defaultEngine}>
                    <option value="auto">Dash 3.0 — Auto · instant, scales up only when needed</option>
                    <option value="2.0">Dash 2.0 — Fast · simple lookups ~10s</option>
                    <option value="1.0">Dash 1.0 — Pro · full team, deepest</option>
                  </select>
                  <button class="emp-btn emp-btn-sm" disabled={engineSaving} onclick={saveDefaultEngine}>{engineSaving ? '◐…' : 'Save model'}</button>
            ...
            <div class="emp-h emp-mt">DEFAULT APPEARANCE</div>
            ...
            <div class="emp-btnrow emp-mt">
              ...
              {#if brandSaved}<span class="emp-saved"><Icon name="check" size={16} /> saved</span>{/if}
              {#if brandErr}<span class="emp-err"><Icon name="x" size={16} /> {brandErr}</span>{/if}
            </div>
          </section>
          ...
          <section class="emp-panel emp-pg-col emp-pg-chatcol">
            <div class="emp-chat-card">
              ...
              <div class="emp-chat-foot">
                ...
                <button class="emp-chat-send" style="background:{brandColor};" disabled aria-label="Send"><Icon name="arrow-right" size={16} /></button>
              </div>
            </div>
        ...
        <section class="emp-panel">
          ...
          {/if}
          <div class="emp-muted emp-fineprint">Change the brand above and all {brandInherit.inherit} inheriting widgets update live — no re-save. Override a single store from its Widget <Icon name="arrow-right" size={16} /> Appearance tab.</div>
        </section>
      {/if}
        ...
        <section class="emp-panel">
          ...
          <div class="wg-auth-card" class:wg-auth-open={authCardOpen}>
            <button class="wg-auth-head" onclick={() => authCardOpen = !authCardOpen}>
              <span class="wg-auth-ico"><Icon name="key" size={16} /></span>
            ...
            {#if authCardOpen}
              <div class="wg-auth-body">
                ...
                <div class="wg-auth-bulk">
                  <span class="wg-auth-warn"><Icon name="alert-triangle" size={16} /> Existing widgets keep their current mode.</span>
          ...
          <div class="wg-tabbar">
            <div class="wg-tabs" role="tablist">
              <button class="wg-tab" class:wg-tab-on={embedTab === 'outlet'} role="tab" aria-selected={embedTab === 'outlet'}
                ...
                <span class="wg-tab-ico"><Icon name="store" size={16} /></span> Outlet
            ...
            <div class="emp-h-actions">
              {#if embeds.length > 0}
                <button class="emp-btn" onclick={downloadDeployAll} title="One zip with every store's ready-to-deploy widget folder (keys baked in) + an INDEX page"><Icon name="arrow-down" size={16} /> Export all</button>
              {/if}
            </div>
          </div>
          ...
          {#if embedTab === 'outlet'}
            <div class="wg-banner">
              <span class="wg-banner-ico"><Icon name="lock" size={16} /></span>
          ...
          {#if newEmbedOpen}
            <div class="emp-mint">
              ...
              <div class="emp-mint-actions">
                ...
                {#if newEmbedErr}<span class="emp-err"><Icon name="x" size={16} /> {newEmbedErr}</span>{/if}
              </div>
              ...
              {#if newEmbedKey}
                <div class="emp-minted">
                  ...
                  <div class="emp-minted-row">
                    ...
                    <button class="emp-btn" onclick={() => copyText(newEmbedKey, 'emkey')}>{copied === 'emkey' ? 'OK copied' : 'copy'}</button>
                  </div>
                </div>
              {/if}
            </div>
          ...
          {:else if embedsErr}
            <div class="emp-row emp-err"><Icon name="x" size={16} /> {embedsErr}</div>
          {:else if tabEmbeds.length === 0}
            ...
            {:else}
              <div class="wg-empty">
                <div class="wg-empty-ico"><Icon name="store" size={16} /></div>
              ...
              <tbody>
                {#each tabEmbeds as e (e.embed_id || e.id)}
                  ...
                  <tr class="emp-row-click wg-row" class:emp-row-open={isOpen} class:wg-row-off={!isLive} onclick={() => toggleRow(eid)} title="Expand — keys, snippet, full PHP code + deploy">
                    ...
                    <td>
                      {#if isStore}
                        <span class="emp-lock" title="Outlet widget tied to a store in the database — permanent, cannot be deleted"><Icon name="lock" size={16} /> Locked</span>
                      {:else}
                        <button class="emp-btn emp-btn-sm emp-btn-danger"
                          title="Delete this widget permanently (double confirmation)"
                          onclick={(ev) => { ev.stopPropagation(); deleteEmbed(e); }}><Icon name="trash" size={16} /> Delete</button>
                      {/if}
                    </td>
                  </tr>
                      ...
                      <td colspan="7" onclick={(ev) => ev.stopPropagation()}>
                        <div class="emp-detail">
                          ...
                          <div class="emp-detail-line">
                            ...
                            <button class="emp-btn emp-btn-sm" onclick={() => copyText(eid, `eid-${eid}`)}>{copied === `eid-${eid}` ? 'OK' : 'copy'}</button>
                          </div>
                          <div class="emp-detail-line">
                          ...
                          <div class="emp-detail-line">
                            ...
                            {#if secretShown[eid] !== undefined}
                              ...
                              {#if secretShown[eid]}<button class="emp-btn emp-btn-sm" onclick={() => copyText(secretShown[eid], `sk-${eid}`)}>{copied === `sk-${eid}` ? 'OK' : 'copy'}</button>{/if}
                            ...
                            {/if}
                            <span class="emp-muted emp-fineprint">server-side only <Icon name="arrow-right" size={16} /> set <code class="emp-code">CITYAGENT_EMBED_SECRET</code> (HMAC mode)</span>
                          </div>
                          <div class="emp-detail-line">
                          ...
                          <div class="emp-dl-snip">
                            <div class="emp-dl-snip-head">
                              ...
                              <button class="emp-btn emp-btn-sm" onclick={() => copyText(buildSnippet(eid, e.public_key || '', scope), `dsnip-${eid}`)}>{copied === `dsnip-${eid}` ? 'OK copied' : 'copy'}</button>
                            </div>
                          ...
                          <div class="emp-dl-snip">
                            <div class="emp-dl-snip-head">
                              ...
                              {#if phpCode[`${eid}:${phpTabOf(eid)}`]}
                                <button class="emp-btn emp-btn-sm" onclick={() => copyText(phpCode[`${eid}:${phpTabOf(eid)}`], `php-${eid}`)}>{copied === `php-${eid}` ? 'OK copied' : 'copy'}</button>
                              {/if}
                            </div>
      ...
      {#if view === 'widget'}
        {#if !configEmbed}
          <section class="emp-panel">
            <div class="emp-row emp-muted">no widget selected — <button class="emp-btn emp-btn-sm" onclick={() => nav('widgets')}><Icon name="arrow-left" size={16} /> back to Widgets</button></div>
          </section>
        {:else}
          ...
          <section class="emp-panel emp-wc-head">
            <div class="emp-wc-crumb">
              ...
              {#if ceLive}<span class="emp-badge emp-badge-on">● live</span>{:else}<span class="emp-badge emp-badge-off"><Icon name="x" size={16} /> not live</span>{/if}
          ...
          {#if wTab === 'appearance'}
            ...
            {#if apprMode === 'inherit'}
              <div class="emp-inherit-card">
                ...
                <div class="emp-btnrow emp-mt">
                  <button class="emp-btn emp-btn-sm" onclick={() => nav('brand')}>Edit Brand defaults <Icon name="arrow-right" size={16} /></button>
                  {#if configEmbed.primary_color}
                    ...
                    {#if configSaved}<span class="emp-saved">OK now inheriting</span>{/if}
                  {/if}
            ...
            <div class="emp-config-grid">
              ...
              <label class="emp-field emp-field-full">
                ...
                {#if logoErr}<span class="emp-err emp-logo-err">x {logoErr}</span>{/if}
            ...
            <div class="emp-btnrow emp-mt">
              ...
              {#if configSaved}<span class="emp-saved">OK saved</span>{/if}
              {#if configErr}<span class="emp-err">x {configErr}</span>{/if}
            </div>
            {/if}
            ...
            <label class="emp-field emp-field-full">
              ...
              <div class="emp-color-row">
                <select class="emp-input emp-select" disabled={configEngineBusy} value={configEngine} onchange={(e) => saveConfigEngine((e.currentTarget as HTMLSelectElement).value as ('' | '2.0' | '1.0'))}>
                  <option value="">Inherit global ({defaultEngine === 'auto' ? 'Dash 3.0' : defaultEngine === '2.0' ? 'Dash 2.0' : 'Dash 1.0'})</option>
          ...
          {#if wTab === 'deploy'}
            ...
            <div class="emp-detail">
              <div class="emp-detail-line">
                ...
                <button class="emp-btn emp-btn-sm" onclick={() => copyText(ceId, `eid-${ceId}`)}>{copied === `eid-${ceId}` ? 'OK' : 'copy'}</button>
              </div>
              <div class="emp-detail-line">
                <span class="emp-dl-k">PUBLIC KEY</span>
                <code class="emp-code emp-code-key">{cePk || '—'}</code>
                {#if cePk}<button class="emp-btn emp-btn-sm" onclick={() => copyText(cePk, `pk-${ceId}`)}>{copied === `pk-${ceId}` ? 'OK' : 'copy'}</button>{/if}
            ...
            <div class="emp-dl-snip-head">
              ...
              <button class="emp-btn emp-btn-sm" onclick={() => copyText(buildSnippet(ceId, cePk, ceScope), `dsnip-${ceId}`)}>{copied === `dsnip-${ceId}` ? 'OK copied' : 'copy'}</button>
            </div>
          ...
          {#if wTab === 'share'}
            ...
            {#if sbLink}
              <div class="emp-codeblock emp-mt">
                <button class="emp-copybtn" onclick={() => copyText(sbLink, 'sblink')}>{copied === 'sblink' ? 'OK' : 'copy'}</button>
        ...
        <section class="emp-panel emp-wd-headpanel">
          <div class="emp-wd-headbar">
            ...
            <div class="emp-wd-headmeta">
              ...
              {#if wh.enabled === false}<span class="emp-err">x disabled</span>{/if}
            ...
            <div class="emp-wd-headact">
              ...
              <button class="emp-btn emp-btn-sm" onclick={wdExportJson} disabled={!wdData}>v JSON</button>
            </div>
          </div>
        </section>

        {#if wdErr}
          <section class="emp-panel"><div class="emp-row emp-err">x widget detail unavailable ({wdErr})</div></section>
          ...
          <div class="emp-mon-2col">
            ...
            <section class="emp-panel">
              ...
              {#if !wdData.errors || Number(wdData.errors.total) === 0}
                <div class="emp-row emp-muted">OK no errors in window</div>
              {:else}
                ...
                <tbody>
                  {#each wdData.calls as c (c.id)}
                    <tr class="emp-mon-row" onclick={() => toggleWdCall(c.id)}>
                      ...
                      <td>{#if c.success}<span class="emp-wd-ok">OK</span>{:else}<span class="emp-err" title={c.error || 'error'}>x</span>{/if}</td>
      ...
      {:else if view === 'monitoring'}
        ...
        <section class="emp-panel">
          <div class="emp-h-row">
            ...
            <div class="emp-btnrow">
              <button class="emp-btn emp-btn-sm" onclick={monExportCsv} disabled={!monData}>v CSV</button>
              <button class="emp-btn emp-btn-sm" onclick={monExportJson} disabled={!monData}>v JSON</button>
            </div>
          </div>
        ...
        {#if monErr}
          <section class="emp-panel"><div class="emp-row emp-err">x monitoring unavailable ({monErr})</div></section>
      ...
      {#if view === 'developer'}
        ...
        <section class="emp-panel">
          ...
          <div class="emp-codeblock">
            <button class="emp-copybtn" onclick={() => copyText(docSnippet, 'doc')}>{copied === 'doc' ? 'OK' : 'copy'}</button>
              ...
              {#each sdkFiles as f}
                ...
                {#if sdkPreviewName === f.name}
                  <div class="emp-codeblock emp-sdk-preview">
                    <button class="emp-copybtn" onclick={() => copyText(sdkPreviewBody, `sdk-${f.name}`)}>{copied === `sdk-${f.name}` ? 'OK' : 'copy'}</button>
...
<style>
 ...
 .emp-lock { font-size: 11px; opacity: 0.5; cursor: help; white-space: nowrap; letter-spacing: 0.02em; display: inline-flex; align-items: center; gap: 4px; }
 ...
 .emp-wc-head-actions { display: flex; align-items: center; gap: 12px; flex-wrap: nowrap; overflow-x: auto; }
