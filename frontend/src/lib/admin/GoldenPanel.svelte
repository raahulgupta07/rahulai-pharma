...
<div class="gld-shell">
  ...
  {#if projectSlug && drift}
    <section class="banner" class:has-regressions={drift.drifted_count > 0}>
      ...
      <a href="/admin/accuracy" class="banner-link">View accuracy <Icon name="arrow-right" size={16} /></a>
    </section>
  {/if}

  {#if error}
    <p class="err"><Icon name="alert-triangle" size={16} /> {error}</p>
  {/if}
  ...
  {#if projectSlug}
    ...
    <section class="table-card">
      ...
      {:else if entries.length === 0}
        <p class="muted small">No golden Q&amp;A pairs yet. Click <strong>+ Add Golden Q</strong> to create one, or promote one via <Icon name="thumbs-up" size={16} /> feedback in chat.</p>
      {:else}
        <table>
          <thead>
            <tr>
            ...
            {#each entries as e (e.id)}
              ...
              <tr>
                ...
                <td class="cell-q">
                  ...
                  {:else}{#each ((e as any).keywords || []) as k}<span class="gp-kw">{k}</span>{/each}{#if !((e as any).keywords || []).length}<span class="muted small"><Icon name="lock" size={16} /></span>{/if}{/if}
                </td>
                <td class="cell-a">
                  {#if e.expected_answer}
                    <div class="exp" title={e.expected_answer}>{truncate(e.expected_answer, 100)}</div>
                  {:else}
                    <span class="muted small"><Icon name="lock" size={16} /> hidden</span>
                  {/if}
                  {#if e.sql}
                ...
                <td>
                  {#if result === 'pass'}
                    <span class="pill pill-pass"><Icon name="check" size={16} /> pass</span>
                  {:else if result === 'fail'}
                    <span class="pill pill-fail"><Icon name="x" size={16} /> fail</span>
                  {:else}
