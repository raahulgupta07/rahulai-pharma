  ...
  {:else if cfg}
    ...
    {:else}
      ...
      <section class="aa-card">
        ...
        <p class="aa-hint">Bind password = env <code>LDAP_APP_PASSWORD</code>. Group<Icon name="arrow-right" size={16} />branch mapping (drives Shop-Counter site binding) is editable in Advanced JSON under <code>ldap.group_to_site</code>.</p>
      </section>
      ...
      <section class="aa-card">
        ...
        {#each cfg.oidc || [] as p, i}
          <div class="aa-prov">
            ...
            <p class="aa-hint">Client secret = env <code>OIDC_{(p.id||'').toUpperCase()}_CLIENT_SECRET</code> (or <code>OAUTH_CLIENT_SECRET</code>). Allowed-roles gate + group<Icon name="arrow-right" size={16} />branch map: edit in Advanced JSON.</p>
...
<style>
 .aap-wrap { color: var(--pw-ink, #1a1a1a); }
