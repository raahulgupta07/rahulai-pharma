<script lang="ts">
 ...
 sbTurns[ai].content = `x HTTP ${r.status}${d ? ' · ' + d : ''}`;
 ...
 sbTurns[ai].content = 'x ' + String(e?.message || e); sbTurns = [...sbTurns];
 }
 sbBusy = false;
 }
...
// $answer holds the full formatted reply (Medicine·Salt·Stock·Price + Tip)`;
 ...
 if (!key) { provTestRes[o] = { content: 'x no key — generate first', meta: '', masked: false, busy: false }; provTestRes = { ...provTestRes }; return; }
 ...
 if (!r.ok) { let d = ''; try { d = (await r.json()).detail || ''; } catch {} provTestRes[o] = { content: `x HTTP ${r.status}${d ? ' · ' + d : ''}`, meta: '', masked: false, busy: false }; provTestRes = { ...provTestRes }; return; }
 const j = await r.json();
 ...
 provTestRes[o] = { content: 'x ' + String(e?.message || e), meta: '', masked: false, busy: false }; provTestRes = { ...provTestRes };
 }
 }
...
{:else if !isSuper}
  <div class="gw-center">
    <div class="gw-denied">
      <div class="gw-denied-mark"><Icon name="x" size={16} /></div>
      ...
      {#if !embedded}
        <button class="gw-btn" onclick={() => goto('/ui/home')}><Icon name="arrow-left" size={16} /> back</button>
      {/if}
    </div>
  </div>
{:else}
    ...
    <main class="gw-main" class:gw-main-wide={view === 'usage' || view === 'outlet'}>
      ...
      {#if view === 'overview'}
        ...
        <section class="gw-panel">
          ...
          {#if statusErr}
            <div class="gw-row gw-err"><Icon name="x" size={16} /> status unavailable ({statusErr})</div>
          {:else if status}
            <div class="gw-status-grid">
              ...
              <div><span class="gw-k">redis</span>{#if (status.redis ?? status.redis_up)}<span class="gw-dot gw-on">●</span> up{:else}<span class="gw-dot gw-off"><Icon name="x" size={16} /></span> down{/if}</div>
        ...
        <section class="gw-panel">
          ...
          <div class="gw-ep">
            ...
            <div class="gw-ep-sub"><span class="gw-muted">stream:false</span> <Icon name="arrow-right" size={16} /> JSON · <span class="gw-muted">stream:true</span> <Icon name="arrow-right" size={16} /> SSE · Auth <code class="gw-code">Bearer dash-key-XXXX</code></div>
          </div>
          <div class="gw-btnrow">
            <button class="gw-btn" onclick={() => copyText(curlSnippet, 'curl')}>{copied === 'curl' ? 'OK copied' : 'copy cURL'}</button>
            <button class="gw-btn" onclick={() => copyText(phpSnippet, 'php')}>{copied === 'php' ? 'OK copied' : 'copy PHP'}</button>
            <button class="gw-btn" onclick={() => copyText(pySnippet, 'py')}>{copied === 'py' ? 'OK copied' : 'copy Python'}</button>
          </div>
        </section>
      {/if}
      ...
      {#snippet sandboxBody()}
        <div class="gw-chat">
            ...
            {#each sbTurns as t, i}
              ...
              {:else}
                <div class="gw-msg gw-msg-bot">
                  <div class="gw-bot-avatar"><Icon name="bot" size={16} /></div>
                  <div class="gw-bot-col">
                      ...
                      {#if t.thinking}
                        ...
                        <div class="gw-steps gw-steps-live">
                          <div class="gw-step-rail">
                            {#each (t.steps || []) as s}
                              <div class="gw-rail-row gw-rail-done" class:gw-step-phase={s.icon === ''}><span class="gw-rail-tick"><Icon name="check" size={16} /></span> {s.label} <span class="gw-step-ic">{s.icon}</span></div>
                            {/each}
                    ...
                    {#if t.meta || t.json || t.curl}
                      <div class="gw-msg-meta">
                        {#if t.masked}<span class="gw-mask-badge"><Icon name="shield" size={16} /> masked</span>{/if}
                      ...
                      {#if t.open}
                        <div class="gw-inspect">
                          ...
                          {#if t.curl}
                            <div class="gw-inspect-head"><span class="gw-inspect-k">cURL</span><button class="gw-btn gw-btn-sm" onclick={() => copyText(t.curl || '', `c-${i}`)}>{copied === `c-${i}` ? 'OK' : 'copy'}</button></div>
                            <pre class="gw-insp-curl">{t.curl}</pre>
                          {/if}
                          {#if t.json}
                            <div class="gw-inspect-head"><span class="gw-inspect-k">raw JSON</span><button class="gw-btn gw-btn-sm" onclick={() => copyText(t.json || '', `j-${i}`)}>{copied === `j-${i}` ? 'OK' : 'copy'}</button></div>
                            <pre class="gw-json">{t.json}</pre>
                          {/if}
                          <div class="gw-insp-note">masking enforced server-side; <Icon name="shield" size={16} /> badge is a client heuristic on sanitizer phrasing.</div>
                        </div>
                      {/if}
                    {/if}
                  </div>
          ...
          {#if sbErr}<div class="gw-sb-err"><Icon name="x" size={16} /> {sbErr}</div>{/if}
      ...
      {#snippet keysBody(showList = true)}
        ...
        <section class="gw-panel">
          ...
          {#if !showList}<div class="gw-muted gw-fineprint" style="margin:-4px 0 8px;">Per-outlet keys <Icon name="arrow-right" size={16} /> <button class="gw-link" onclick={() => nav('provision')}>Outlet Keys</button>. Mint a <strong>global</strong> key here for cross-branch BI / analytics.</div>{/if}
          ...
          {#if mintOpen}
            <div class="gw-mint">
              ...
              <div class="gw-mint-actions">
                ...
                {#if mintErr}<span class="gw-err"><Icon name="x" size={16} /> {mintErr}</span>{/if}
              </div>
              {#if mintedKey}
                <div class="gw-minted">
                  <div class="gw-warn">◐ shown once — copy now, it will not be displayed again</div>
                  <div class="gw-minted-row"><code class="gw-code gw-code-key">{mintedKey}</code><button class="gw-btn" onclick={() => copyText(mintedKey, 'minted')}>{copied === 'minted' ? 'OK copied' : 'copy'}</button></div>
                </div>
              {/if}
            </div>
          {/if}

          {#if keysErr}<div class="gw-row gw-err"><Icon name="x" size={16} /> keys unavailable ({keysErr})</div>
            ...
            <div class="gw-outlet-grid">
              {#each outletPage as o (o)}
                ...
                <button class="gw-outlet" class:gw-outlet-bound={bound} onclick={() => copyText(o, `o-${o}`)} title={bound ? 'covered by a key · click to copy' : 'not bound to any key · click to copy'}>
                  ...
                  <code class="gw-code">{o}</code>{#if copied === `o-${o}`}<span class="gw-saved"> <Icon name="check" size={16} /></span>{/if}
                </button>
              {/each}
            </div>
        ...
        <section class="gw-panel">
          ...
          <div class="gw-prov-banner">
            ...
            <div class="gw-prov-rate">
              ...
              {#if rateSaved}<span class="gw-saved"><Icon name="check" size={16} /></span>{/if}
              {#if rateErr}<span class="gw-err"><Icon name="x" size={16} /> {rateErr}</span>{/if}
            </div>
          </div>
          ...
          <div class="gw-prov-tools">
            ...
            <div class="gw-prov-export">
              ...
              <button class="gw-btn gw-btn-sm" title="One key-agnostic client (php + py + README) that serves every shop. Drop your .env in and run." onclick={() => { const origin = baseUrl.replace(/\/api\/v1\/?$/, ''); window.open(`${origin}/api/embed/sdk/gateway-bundle.zip?base=${encodeURIComponent(baseUrl)}`, '_blank'); }}><Icon name="arrow-down" size={16} /> Bundle .zip</button>
            </div>
          </div>

          {#if provErr}<div class="gw-row gw-err"><Icon name="x" size={16} /> provision unavailable ({provErr})</div>{/if}
              ...
              {#each provView as r (r.outlet)}
                ...
                <div class="gw-prov-row" class:gw-prov-open={provExpanded === r.outlet}>
                  ...
                  <span class="gw-prov-cell">
                    ...
                    {:else}<span class="gw-prov-warn"><Icon name="alert-triangle" size={16} /> no key</span>{/if}
                  </span>
                  ...
                  <span class="gw-prov-cell">
                    {#if r.keyed}<button class="gw-btn gw-btn-sm" onclick={() => openChat(r.outlet, r.name)}><Icon name="play" size={16} /> test</button>{:else}<span class="gw-muted">—</span>{/if}
                  </span>
                  <span class="gw-prov-cell">
                    {#if r.keyed}
                      <button class="gw-btn gw-btn-sm" onclick={async () => { const k = await provReveal(r.outlet, r.name); if (k) copyText(k, `pk-${r.outlet}`); }}>{copied === `pk-${r.outlet}` ? 'OK' : (provRevealBusy === r.outlet ? '◐' : ' key')}</button>
                    {:else}
                  ...
                  <div class="gw-prov-detail">
                    {#if r.keyed}
                      <div class="gw-prov-drow">
                        ...
                        <button class="gw-btn gw-btn-sm" onclick={() => provKeys[r.outlet] && copyText(provKeys[r.outlet], `pkf-${r.outlet}`)}>{copied === `pkf-${r.outlet}` ? 'OK' : ''}</button>
                        <button class="gw-btn gw-btn-sm" disabled={provBusy} onclick={() => provRotate(r.outlet)}><Icon name="refresh" size={16} /> rotate</button>
                      ...
                      <div class="gw-prov-snip">
                        <div class="gw-prov-langtabs">
                          ...
                          {/each}
                          <button class="gw-btn gw-btn-sm gw-prov-snipcopy" onclick={() => copyText(provSnippet(r.outlet, provLang[r.outlet] || 'php'), `ps-${r.outlet}`)}>{copied === `ps-${r.outlet}` ? 'OK copied' : ' copy'}</button>
                        </div>
                      ...
                      <div class="gw-prov-test">
                        <button class="gw-btn gw-btn-sm gw-btn-accent" onclick={() => openChat(r.outlet, r.name)}><Icon name="message-square" size={16} /> Open chat test</button>
          ...
          <div class="gw-drawer" role="dialog" aria-label="Outlet chat test" tabindex="-1">
            <div class="gw-drawer-head">
              <div class="gw-drawer-id">
                <span class="gw-drawer-av"><Icon name="store" size={16} /></span>
                <div>
                  <div class="gw-drawer-title">CityPharma · <code class="gw-code">{chatOutlet}</code></div>
                  <div class="gw-drawer-sub"><span class="gw-dot gw-on">●</span> online · key bound · <span class="gw-mask-badge"><Icon name="shield" size={16} /> masked live</span></div>
                </div>
              </div>
              <button class="gw-drawer-x" onclick={closeChat} aria-label="Close"><Icon name="x" size={16} /></button>
            </div>
            ...
            <div class="gw-drawer-body" bind:this={sbBodyEl}>
              {#if chatKeyMissing}
                <div class="gw-chat-empty gw-err"><Icon name="x" size={16} /> could not reveal key for {chatOutlet}. Generate / rotate it first.</div>
                    ...
                    <div class="gw-bot-col">
                      ...
                      {#if t.meta || t.json || t.curl}
                        ...
                        {#if t.open}
                          <div class="gw-inspect">
                            {#if t.curl}
                              <div class="gw-inspect-head"><span class="gw-inspect-k">cURL</span><button class="gw-btn gw-btn-sm" onclick={() => copyText(t.curl || '', `dc-${i}`)}>{copied === `dc-${i}` ? 'OK' : 'copy'}</button></div>
                              <pre class="gw-insp-curl">{t.curl}</pre>
                            {/if}
                            {#if t.json}
                              <div class="gw-inspect-head"><span class="gw-inspect-k">raw JSON</span><button class="gw-btn gw-btn-sm" onclick={() => copyText(t.json || '', `dj-${i}`)}>{copied === `dj-${i}` ? 'OK' : 'copy'}</button></div>
                              <pre class="gw-json">{t.json}</pre>
                            {/if}
                          </div>
                        {/if}
      ...
      {#if view === 'usage'}
        ...
        <section class="gw-panel">
          <div class="gw-an-head">
            ...
            <div class="gw-an-exports">
              <button class="gw-btn gw-btn-sm" onclick={anExportCsv} disabled={!overview}><Icon name="arrow-down" size={16} /> CSV</button>
              <button class="gw-btn gw-btn-sm" onclick={anExportJson} disabled={!overview}><Icon name="arrow-down" size={16} /> JSON</button>
            </div>
          </div>
        ...
        {#if ovErr}
          <section class="gw-panel"><div class="gw-row gw-err"><Icon name="x" size={16} /> analytics unavailable ({ovErr})</div></section>
        ...
        {:else if overview}
            ...
            <div class="gw-kpi-grid">
              {#snippet kpi(label: string, value: any, sub: string = '', delta: number | null = null, warn: boolean = false)}
                <div class="gw-kpi" class:gw-kpi-warn={warn}>
                  ...
                  <div class="gw-kpi-val">{value}{#if warn} <span class="gw-err"><Icon name="alert-triangle" size={16} /></span>{/if}</div>
            ...
            <section class="gw-panel">
              ...
              {#if costOff}
                <div class="gw-soft-card">
                  ...
                  <div class="gw-muted gw-soft-note"><Icon name="settings" size={16} /> model pricing not set — set per-model rates to see spend over time.</div>
                </div>
                ...
                <div class="gw-hbars">
                  {#each ovLatHist as h}
                    <div class="gw-hbar-row" class:gw-hbar-tail={isTail(h.bucket)}>
                      <div class="gw-hbar-lbl">{h.bucket}{#if isTail(h.bucket)} <span class="gw-warn"><Icon name="alert-triangle" size={16} /></span>{/if}</div>
                ...
                <tbody>
                  {#each ovByModel as m (m.model)}
                    ...
                    <tr>
                      ...
                      <td>{m.errors ?? 0}{#if Number(m.errors) > 0} <span class="gw-err"><Icon name="alert-triangle" size={16} /></span>{/if}</td>
          ...
          <section class="gw-panel">
            <div class="gw-h-row"><div class="gw-subh">By key</div><button class="gw-btn gw-btn-sm" onclick={anExportCsv}><Icon name="arrow-down" size={16} /> CSV</button></div>
                ...
                <tbody>
                  {#each ovByKey as row (row.key)}
                    <tr>
                      ...
                      <td>{row.errors ?? 0}{#if Number(row.errors) > 0} <span class="gw-err"><Icon name="alert-triangle" size={16} /></span>{/if}</td>
                ...
                <tbody>
                  {#each ovByStore as s (s.store)}
                    <tr>
                      ...
                      <td>{s.errors ?? 0}{#if Number(s.errors) > 0} <span class="gw-err"><Icon name="alert-triangle" size={16} /></span>{/if}</td>
          ...
          <section class="gw-panel">
            ...
            {#if !ovErrors || (Number(ovErrors.total) === 0)}
              <div class="gw-row gw-muted"><Icon name="check" size={16} /> no errors in window — {((Number(ovKpi.error_rate) || 0) * 100).toFixed(1)}% rate</div>
            {:else}
              <div class="gw-status-grid">
      ...
      {#if view === 'outlet'}
        ...
        <section class="gw-panel gw-ol-headpanel">
          <div class="gw-ol-headbar">
            <button class="gw-btn gw-btn-sm gw-ol-back" onclick={backToAnalytics}><Icon name="arrow-left" size={16} /> Back to Analytics</button>
            ...
            <div class="gw-ol-headact">
              ...
              <button class="gw-btn gw-btn-sm" onclick={outletExportJson} disabled={!outletOv}><Icon name="arrow-down" size={16} /> JSON</button>
            </div>
          </div>
        </section>

        {#if outletErr}
          <section class="gw-panel"><div class="gw-row gw-err"><Icon name="x" size={16} /> outlet detail unavailable ({outletErr})</div></section>
          ...
          <div class="gw-an-2col">
            ...
            <section class="gw-panel">
              ...
              {#if !outErrors || (Number(outErrors.total) === 0)}
                <div class="gw-row gw-muted"><Icon name="check" size={16} /> no errors in window</div>
              {:else}
                <div class="gw-status-grid">
                ...
                <tbody>
                  {#each outQuestions as q (q.session_id + '|' + q.ts)}
                    ...
                    <tr class="gw-ol-qrow" class:gw-ol-qrow-on={outletQExpanded === q.session_id} onclick={() => toggleOutletQ(q.session_id)}>
                      ...
                      <td class="gw-ol-flags">{#if q.masked}<span title="masked (store tier)"><Icon name="shield" size={16} /></span>{/if}{#if err}<span class="gw-err" title={String(err)}><Icon name="x" size={16} /></span>{:else}<span class="gw-ol-ok" title="ok"><Icon name="check" size={16} /></span>{/if}</td>
                    </tr>
      ...
      {#if view === 'docs'}
        <div class="gw-doc-banner">
          ...
          <a class="gw-doc-link" href="/api/v1/docs" target="_blank" rel="noopener">Open full docs <Icon name="trending-up" size={16} /></a>
        </div>
        ...
        <section class="gw-panel">
          ...
          <div class="gw-codeblock"><button class="gw-copybtn" onclick={() => copyText(reqSchema, 'req')}>{copied === 'req' ? 'OK' : 'copy'}</button><pre class="gw-pre">{reqSchema}</pre></div>
          <div class="gw-subh gw-mt">Response — blocking</div>
          <div class="gw-codeblock"><button class="gw-copybtn" onclick={() => copyText(respSchema, 'resp')}>{copied === 'resp' ? 'OK' : 'copy'}</button><pre class="gw-pre">{respSchema}</pre></div>
          <div class="gw-subh gw-mt">Response — streaming chunks</div>
          <div class="gw-codeblock"><button class="gw-copybtn" onclick={() => copyText(chunkSchema, 'chunk')}>{copied === 'chunk' ? 'OK' : 'copy'}</button><pre class="gw-pre">{chunkSchema}</pre></div>
        </section>

        <section class="gw-panel">
          <div class="gw-h">STREAMING</div>
          <p class="gw-doc-p">Set <code class="gw-code">"stream": true</code> <Icon name="arrow-right" size={16} /> <code class="gw-code">text/event-stream</code> of <code class="gw-code">chat.completion.chunk</code> frames, terminated by <code class="gw-code">data: [DONE]</code>.</p>
          <div class="gw-codeblock"><button class="gw-copybtn" onclick={() => copyText(streamCurlSnippet, 'scurl')}>{copied === 'scurl' ? 'OK' : 'copy'}</button><pre class="gw-pre">{streamCurlSnippet}</pre></div>
        </section>
...
<style>
 ...
 .gw-rail-done .gw-rail-tick { color: #5b8c63; font-weight: 700; } /* greenish-muted OK */
