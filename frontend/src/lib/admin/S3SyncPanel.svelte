<script lang="ts">
 ...
 testResult = d.ok
 ? `OK CONNECTED — ${d.objects} object(s), ${d.matched} match your patterns. Sample: ${(d.sample || []).join(', ')}`
 : `x ${d.error}`;
 } finally { busy = false; }
 }
...
<div class="s3">
  <div class="s3-head">
    <div>
      <div class="s3-title"><Icon name="cloud" size={16} /> S3 SYNC</div>
      <div class="s3-sub">Auto-pull from S3 <Icon name="arrow-right" size={16} /> replace tables <Icon name="arrow-right" size={16} /> retrain. Connect one or more buckets.</div>
    </div>
  ...
  {#if !boto3}
    <div class="warn"><Icon name="alert-triangle" size={16} /> S3 LIBRARY NOT INSTALLED (boto3 missing). Rebuild the image with boto3 in requirements.</div>
  {/if}
        ...
        {#each sources as s (s.id)}
          <tr>
            ...
            <td class="acts">
              ...
              <button class="ico" title="Edit" onclick={() => editSource(s)}>(edit)</button>
              <button class="ico danger" title="Delete" onclick={() => remove(s.id)}><Icon name="x" size={16} /></button>
            </td>
          </tr>
        {/each}
      </tbody>
...
{#if editing}
  <div class="modal-bg" role="presentation" onclick={() => editing = null}>
    <div class="modal" role="dialog" aria-modal="true" onclick={(e) => e.stopPropagation()}>
      <div class="modal-h"><Icon name="cloud" size={16} /> {editing.id ? 'EDIT' : 'ADD'} S3 ACCOUNT</div>
      <div class="modal-body">
        ...
        <div class="rule-hd">FILE <Icon name="arrow-right" size={16} /> TABLE RULES <button class="sq ghost xs" onclick={addRule}>+ RULE</button></div>
        <div class="rule-row hdr"><span>FILE PATTERN</span><span><Icon name="arrow-right" size={16} /> TABLE</span><span>MODE</span><span></span></div>
        {#each editing.file_map as r, i}
          <div class="rule-row">
            ...
            <button class="ico danger" onclick={() => delRule(i)}><Icon name="x" size={16} /></button>
          </div>
        {/each}
...
{#if detail}
  <div class="modal-bg" role="presentation" onclick={() => detail = null}>
    <div class="modal" role="dialog" aria-modal="true" onclick={(e) => e.stopPropagation()}>
      <div class="modal-h"><Icon name="cloud" size={16} /> {detail.name} — LAST RUN LOG <button class="ico" style="margin-left:auto" onclick={() => detail = null}><Icon name="x" size={16} /></button></div>
      <div class="modal-body">
