<script lang="ts">
 ...
 if (d.ok) flash('ok', `OK valid · ${d.model_count} models`);
 else flash('err', `x ${d.error || d.status}`);
 }
...
<div class="llm-shell">
  ...
  <div class="llm-sync-status">
    ...
    {#if !syncing}
      <button class="llm-link-btn" onclick={syncModels}><Icon name="refresh" size={16} /> Sync now</button>
    {/if}
  </div>
  ...
  {#if error}
    <div class="llm-err"><Icon name="alert-triangle" size={16} /> {error}</div>
  {/if}
                  ...
                  <div class="llm-edit-panel">
                    ...
                    <div class="llm-edit-replace">
                      ...
                      {#if editReplaceExpanded}
                        ...
                        />
                        <div class="llm-warn">
                          <Icon name="alert-triangle" size={16} /> Replacing the key will reset usage counters. Old key revoked.
                        </div>
                      {/if}
                    </div>
