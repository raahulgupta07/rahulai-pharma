...
<div class="obs-root">
  ...
  <section class="obs-strip obs-strip-ctx">
    ...
    {#if ctxHealth}
      {#if true}
        ...
        <span class="obs-sep">/</span>
        <span class="obs-stat" class:obs-warn={isFinite(p95) && p95 > NEAR}>p95 <b>{fmtTok(ctxHealth.prompt_tokens_p95)}</b>{#if isFinite(p95) && p95 > NEAR}<span class="obs-warnmark" title={`approaching ${BUDGET} budget`}> <Icon name="alert-triangle" size={16} /></span>{/if}</span>
        <span class="obs-sep">·</span>
  ...
  {#if error}
    <div class="obs-err-banner"><Icon name="x" size={16} /> traces unavailable ({error})</div>
  {/if}
      ...
      <div class="obs-tracelist">
        {#each traces as t (t.trace_id ?? t.name)}
          ...
          <div class="obs-trace" class:obs-trace-err={t.status === 'error'}>
            ...
            {#if open}
              {#if t.error}
                <div class="obs-error-text"><Icon name="x" size={16} /> {t.error}</div>
              {/if}
              {#if kids.length}
                <div class="obs-children">
                  {#each kids as c, ci (ci)}
                    <div class="obs-child" class:obs-child-err={c.status === 'error'}>
                      ...
                      {#if c.error}<div class="obs-error-text obs-child-error"><Icon name="x" size={16} /> {c.error}</div>{/if}
                    </div>
                  {/each}
                </div>
              {:else}
