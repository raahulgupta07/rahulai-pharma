...
<div class="lf">
  <div class="lf-h"><Icon name="bot" size={16} /> LEARNING FEED <span class="lf-live"><span class="lf-dot"></span>live</span></div>
  {#if !items.length}
      ...
      {#each items as it}
        <li class="lf-row">
          ...
          <span class="lf-meta">
            ...
            <span class="lf-tag {it.status}">{it.status === 'cemented' ? 'OK cemented' : it.status === 'review' ? ' review' : 'x dropped'}</span>
          </span>
        </li>
      {/each}
    </ul>
