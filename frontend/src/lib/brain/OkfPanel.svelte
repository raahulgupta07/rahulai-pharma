...
<div class="okf">
  <div class="okf-head">
    <div>
      ...
      <div class="okf-sub">Import a knowledge bundle <Icon name="arrow-right" size={16} /> review <Icon name="arrow-right" size={16} /> promote to live. Export the current knowledge as a portable bundle.</div>
    </div>
    <div class="okf-actions">
      <button class="okf-btn" disabled={busy} onclick={exportBundle}><Icon name="arrow-down" size={16} /> Export bundle</button>
      <button class="okf-btn okf-primary" disabled={busy} onclick={() => fileEl?.click()}><Icon name="arrow-up" size={16} /> Import bundle (.zip)</button>
  ...
  <div class="okf-bar">
    ...
    <button class="okf-btn" disabled={busy} onclick={loadPending}><Icon name="refresh" size={16} /> Refresh</button>
    <button class="okf-btn okf-reject" disabled={busy || pending.total === 0} onclick={reject}><Icon name="x" size={16} /> Reject all</button>
    <button class="okf-btn okf-approve" disabled={busy || pending.total === 0} onclick={promote}><Icon name="check" size={16} /> Promote to live</button>
  </div>
  ...
  {:else}
    {#if pending.facts.length}
      ...
      {#each pending.facts as f (f.id)}<div class="okf-row"><Icon name="brain" size={16} /> {f.text}</div>{/each}
    {/if}
    {#if pending.queries.length}
      <div class="okf-group">Verified Queries ({pending.queries.length})</div>
      {#each pending.queries as q (q.id)}<div class="okf-row"><Icon name="search" size={16} /> <b>{q.question}</b><pre>{q.sql}</pre></div>{/each}
    {/if}
    {#if pending.triples.length}
      <div class="okf-group">Relationships ({pending.triples.length})</div>
      {#each pending.triples as t (t.id)}<div class="okf-row"><Icon name="network" size={16} /> {t.text}</div>{/each}
    {/if}
  {/if}
</div>
