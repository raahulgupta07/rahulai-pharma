...
<div class="cx">
  <!-- hero -->
  <div class="cx-hero">
    <span class="cx-brain"><Icon name="brain" size={16} /></span>
    <span class="cx-title">CORTEX</span>
    <span class="cx-live"><span class="cx-dot"></span>LIVE</span>
    <div class="cx-vit">
      <span><b>{totalMem}</b> memories</span>
      <span><Icon name="heart" size={16} /> <b>{freshness}%</b> healthy</span>
      <span><Icon name="alert-triangle" size={16} /> <b>{blindSpots}</b> blind</span>
  ...
  <div class="cx-panel">
    {#if mode === 'anatomy'}
      <svg viewBox="0 0 760 430" width="100%" height="430">
        ...
        {#each REGIONS as rg}
          ...
          <g class="cx-lobe" role="button" tabindex="0" onclick={() => jump(rg.jump)} onkeydown={(e) => e.key === 'Enter' && jump(rg.jump)}>
            ...
            {/if}
            <text class="cx-ll" x={rg.cx} y={rg.cy - 14} text-anchor="middle">{rg.name}{#if rg.hippo} <Icon name="zap" size={16} />{/if}</text>
      ...
      <div class="cx-leg">
        ...
        <span><Icon name="zap" size={16} /> hippocampus — facts forming</span>
        <span>● brainstem — live firing</span>
        <span style="margin-left:auto;color:var(--pw-accent,#c2683f)">click a lobe <Icon name="arrow-right" size={16} /> opens that tab <Icon name="arrow-up" size={16} /></span>
      </div>
        ...
        <div class="cx-lane">
          ...
          {#each lanePending as it}
            <div class="cx-m pending">
              ...
              <div class="cx-mb"><button class="cx-mbtn ok" onclick={() => approveInsight(it.id)}><Icon name="check" size={16} /> approve</button><button class="cx-mbtn no" onclick={() => rejectInsight(it.id)}><Icon name="x" size={16} /> reject</button></div>
            </div>
          {/each}
        </div>
        <div class="cx-lane">
    ...
    {:else}
      ...
      <div class="cx-chips">
        <div class="cx-chip"><div class="cv">{totalMem}</div><div class="ck"><Icon name="heart" size={16} /> memories</div></div>
        ...
        <div class="cx-chip"><div class="cv">{pendCount}</div><div class="ck"><Icon name="clock" size={16} /> forming</div></div>
        <div class="cx-chip"><div class="cv">{blindSpots}</div><div class="ck"><Icon name="alert-triangle" size={16} /> blind spots</div></div>
