...
{#if safeItems.length > 0}
 {#if collapsed}
  <div class="rt-collapsed" onclick={toggle} role="button" tabindex="0"
   ...
   <span class="rt-clock"><Icon name="clock" size={16} /> {elapsedS}s</span>
   <span class="rt-dot">·</span>
   <span class="rt-meta"><Icon name="arrow-up" size={16} />{totals.tin} <Icon name="arrow-down" size={16} />{totals.tout}</span>
   <span class="rt-dot">·</span>
 ...
 {:else}
  <div class="rt-block">
   <div class="rt-header" onclick={toggle} role="button" tabindex="0"
    ...
    <span class="rt-clock"><Icon name="clock" size={16} /> {elapsedS}s</span>
    <span class="rt-sep">·</span>
    <span class="rt-tok"><Icon name="arrow-up" size={16} />{totals.tin} <Icon name="arrow-down" size={16} />{totals.tout}</span>
    <span class="rt-sep">·</span>
   ...
   <div class="rt-body">
    {#each folded as entry (entry.item.id + '_' + entry.idx)}
     ...
     {:else}
      ...
      {#if entry.corrected}
       <div class="rt-sub rt-corrected"><Icon name="refresh" size={16} /> auto-corrected one query — retried clean.</div>
      {/if}
      ...
      {:else if resultSummary(entry.item)}
       <div class="rt-sub rt-result"><Icon name="arrow-right" size={16} /> {resultSummary(entry.item)}</div>
      {/if}
   ...
   <div class="rt-foot">total · {nSteps} steps · <Icon name="arrow-up" size={16} />{totals.tin} <Icon name="arrow-down" size={16} />{totals.tout} · ${totals.cost.toFixed(4)}</div>
  </div>
 {/if}
{/if}
