<script lang="ts">
 ...
 const clean = (text || '').replace(/^[OKx·]\s*/, '').trim();
 ...
 if (/error|fail|x|x/i.test(text)) return 'rl-err';
 if (/warn|warning|/i.test(text)) return 'rl-warn';
 if (/OK|done|complete|success/i.test(text)) return 'rl-ok';
 ...
 trainStatus === 'done' ? 'OK DONE' :
 trainStatus === 'error' ? 'x ERROR' :
  ...
  {:else}
    ...
    <div class="rp-panel">
      ...
      <div class="rp-head">
        ...
        <button class="rp-close" onclick={toggle} aria-label="Close"><Icon name="x" size={16} /></button>
      </div>
          ...
          {#if roster.done.length}
            ...
            {#each roster.done as a (a.id)}
              <div class="rp-ag rp-ag-done">
                <span class="rp-ag-x"><Icon name="check" size={16} /></span>
        ...
        {:else if tab === 'history'}
          ...
          {:else}
            {#each histByDate as day (day._k)}
              <div class="rp-datehdr"><Icon name="calendar" size={16} /> {day.date}</div>
