<script lang="ts">
 ...
 const STEP_ICON: Record<string, string> = { done: 'OK', running: '◉', error: 'x', warn: '', gated: '⊘', idle: '·' };
...
<div class="tf">
  ...
  <div class="tf-hd">
    ...
    {#if phase === 'done'}
      <span class="tf-badge tf-badge-done"><Icon name="check" size={16} /> DONE</span>
    ...
    {:else if phase === 'failed'}
      <span class="tf-badge tf-badge-fail"><Icon name="x" size={16} /> FAILED</span>
    {:else}
    ...
    <div class="tf-schem">
      {#each FLOW_LAYERS as L, li}
        ...
        <div class="tf-scard tf-sc-{ls}" class:tf-on={drilled === L.idx} style="--c:{c};--ci:{ink(c)};grid-row:{gridRow(li)};grid-column:{gridCol(li)};" onclick={() => drill(L.idx)} role="button" tabindex="0">
          <div class="tf-st"><span>L{li}</span><span class="tf-ck">{ls === 'done' ? 'OK' : ''}</span></div>
          <div class="tf-sl">{L.short}</div>
          <div class="tf-sd">{L.steps[0]?.label}{L.steps.length > 1 ? '\n' + L.steps[1].label : ''}</div>
          <div class="tf-sf">{liveStepCount(L)} steps · <Icon name="arrow-right" size={16} /> {LTAGS[li]}</div>
          {#if dir !== 'none'}
        ...
        <div id="tf-ly-{L.idx}" class="tf-layer" class:tf-ly-on={drilled === L.idx}>
          ...
          {#each (L.steps ?? []) as st}
            ...
            <div class="tf-step tf-s-{ss}">
              ...
              {#if wk}
                <button type="button" class="tf-wr tf-wr-link" onclick={() => openStore(storeByKey(wk))} title="open {wt} detail"><Icon name="arrow-right" size={16} /> {wt}</button>
              {:else}
                <span class="tf-wr"><Icon name="arrow-right" size={16} /> {wt}</span>
              {/if}
            </div>
          {/each}
        </div>
  ...
  <!-- legend -->
  <div class="tf-legend">
    states: ◉ running · <Icon name="check" size={16} /> done · · idle · ⊘ gated &nbsp; models:
...
{#if storeOpen}
  ...
  <div class="tf-modal" role="dialog" aria-modal="true">
    <div class="tf-mhead">
      ...
      <button type="button" class="tf-mx" onclick={closeStore} aria-label="Close"><Icon name="x" size={16} /></button>
    </div>
    <div class="tf-mbody">
      {#if storeLoading}
