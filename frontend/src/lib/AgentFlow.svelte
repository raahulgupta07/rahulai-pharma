...
<div class="aflow" class:running={processing}>
  <div class="aflow-head">
    ...
    <span class="aflow-status"><span class="dot"></span>{processing ? 'processing' : 'idle'}</span>
  </div>
  ...
  <div class="aflow-card">
    ...
    <div class="drop-center"><div class="dash-v"></div></div>
    <div class="bus"><div class="dash-h wide"></div></div>

    <!-- Row 2: 5 stores w/ fan-down stubs -->
    <div class="stub-row">
      {#each stores as _}<div class="dash-v stub"></div>{/each}
    </div>
    <div class="stores">
      {#each stores as s}
        <div class="box store">
...
<style>
  ...
  .aflow-status .dot { width:8px; height:8px; border-radius:50%; background:var(--pw-muted); }
  .aflow.running .aflow-status .dot { background:var(--pw-accent); animation:pulse 1.2s ease-in-out infinite; }
  ...
  .crew-dot { width:7px; height:7px; border-radius:50%; background:var(--pw-accent-soft, #e0b3a3); }
  .aflow.running .crew-dot { background:var(--pw-accent); }
  ...
  .store-fill { height:8px; background:var(--pw-bg); border:1px solid var(--pw-border); margin:6px 0 4px; }
  .store-fill span { display:block; height:100%; background:#9ec79b; }
  ...
  .aflow.running .dash-h { animation:flowx 0.6s linear infinite; }
  .aflow.running .dash-v { animation:flowy 0.6s linear infinite; }
