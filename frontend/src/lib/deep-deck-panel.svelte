...
<div class="dd-panel">
  <header class="dd-header">
    <div class="dd-title">
      <span class="dd-icon"><Icon name="microscope" size={16} /></span>
      <div>
    ...
    <div class="dd-actions">
      {#if presId !== null && finished}
        <button class="dd-btn-primary" onclick={handoff}>OPEN DECK <Icon name="arrow-right" size={16} /></button>
      {/if}
  ...
  {#if !started}
    <div class="dd-launch">
      ...
      <button class="dd-btn-primary dd-btn-launch" onclick={start}><Icon name="play" size={16} /> START PIPELINE</button>
    </div>
  {/if}
    ...
    {#each stageOrder as s, i}
      <div class="dd-stage {stageStatus[s]}">
        <span class="dd-stage-glyph">
          {#if stageStatus[s] === 'done'}<Icon name="check" size={16} />
          {:else if stageStatus[s] === 'running'}●
          {:else if stageStatus[s] === 'waiting'}<Icon name="pause" size={16} />
          {:else if stageStatus[s] === 'error'}<Icon name="x" size={16} />
          {:else}○{/if}
        </span>
    ...
    <div class="dd-approval">
      ...
      <div class="dd-approval-actions">
        <button class="dd-btn-primary" disabled={approving || keptGapIndices.size === 0} onclick={approve}>
          {approving ? 'APPROVING…' : `OK APPROVE (${keptGapIndices.size})`}
        </button>
        <button class="dd-btn-ghost" onclick={() => (showEditModal = !showEditModal)}>
          {showEditModal ? 'DONE EDITING' : '(edit) EDIT'}
        </button>
      </div>
    </div>
  {/if}
    ...
    <div class="dd-live">
      ...
      {#each liveData.executed.slice(0, 6) as e, i}
        <div class="dd-live-row">
          <div class="dd-live-q">
            <span class="dd-live-glyph">{e.ok ? 'OK' : 'x'}</span>
  ...
  {#if errorText}
    <div class="dd-error"><Icon name="alert-triangle" size={16} /> {errorText}</div>
  {/if}
</div>
