 ...
 */
 ...
 break;
 case 'stage_done':
 appendThinking(`OK ${payload?.stage || 'stage'} done`);
 break;
 case 'stage_error':
 appendThinking(`x ${payload?.stage || 'stage'} — ${payload?.error || 'error'}`);
 break;
 case 'panel_ready':
...
<div class="studio-root">
  ...
  <header class="studio-topbar">
    <div class="topbar-left">
      <button class="back-btn" onclick={() => goto(`${base}/project/${encodeURIComponent(slug)}`)} title="Back to project"><Icon name="arrow-left" size={16} /></button>
    ...
    <div class="topbar-right">
              ...
              {#each dashVersions as v}
                <div style="display: flex; align-items: center; gap: 4px; border-bottom: 1px solid var(--pw-border, #e2ddd2);">
                  ...
                  <button type="button"
                          ...
                          style="padding: 4px 8px; margin-right: 4px; background: transparent; color: {dashVersions.length === 1 ? 'var(--pw-muted, #999)' : '#c0392b'}; border: none; cursor: {dashVersions.length === 1 ? 'not-allowed' : 'pointer'}; opacity: {dashVersions.length === 1 ? 0.45 : 1}; font-size: 13px;"><Icon name="x" size={16} /></button>
                </div>
              {/each}
            </div>
          {/if}
      ...
      {#if dashId}
        <button
          ...
          <Icon name="message-square" size={16} /> {chatOpen ? 'Close' : 'Ask'}
        </button>
      {/if}
      <button
      ...
      {#if narrative}
        <header class="exec-overview">
          ...
          {#if typeof narrative.verified_value_count === 'number' && narrative.verified_value_count > 0}
            <div class="exec-verified"><Icon name="check" size={16} /> {narrative.verified_value_count} verified value{narrative.verified_value_count === 1 ? '' : 's'}</div>
          {/if}
        </header>
      {/if}
