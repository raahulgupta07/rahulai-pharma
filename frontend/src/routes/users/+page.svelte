        ...
        <div class="ua-rowgroup" class:open={isOpen}>
          <button class="ua-row" class:ua-inactive={u.is_active === false} onclick={() => toggleRow(u)}>
            <span class="c-user">
              ...
              <span class="ua-id">
                <span class="ua-name">{u.username}{#if u.username === superName}<span class="ua-lock"><Icon name="lock" size={16} /></span>{/if}</span>
            ...
            <div class="ua-detail">
              <div class="ua-grid">
                <section>
                  <h4>Identity</h4>
                  <dl>
                    <dt>User ID</dt><dd>{u.id} <button class="ua-cp" onclick={() => copy(String(u.id), 'id'+u.id)}>{copied === 'id'+u.id ? 'OK' : '⧉'}</button></dd>
                    <dt>Username</dt><dd>{u.username}</dd>
                    <dt>Email</dt><dd>{u.email || '—'}{#if u.email} <button class="ua-cp" onclick={() => copy(u.email, 'em'+u.id)}>{copied === 'em'+u.id ? 'OK' : '⧉'}</button>{/if}</dd>
                ...
                <section>
                  ...
                  <dl>
                    ...
                    <dt>External ID</dt><dd class="ua-mono" title={u.external_id || ''}>{u.external_id || '—'}{#if u.external_id} <button class="ua-cp" onclick={() => copy(u.external_id, 'ex'+u.id)}>{copied === 'ex'+u.id ? 'OK' : '⧉'}</button>{/if}</dd>
                    <dt>Mapped email</dt><dd>{u.email || '—'}</dd>
                    <dt>Site / branch</dt><dd>{u.site_code || '—'}</dd>
                    <dt>Group<Icon name="arrow-right" size={16} />role</dt><dd><a href="{base}/ui/command-center?tab=auth">map in Authentication <Icon name="arrow-right" size={16} /></a></dd>
                  </dl>
                </section>
                <section>
                  <h4>Activity</h4>
                  ...
                  <dl>
                    ...
                    <dd>
                      {#if amSuper && u.username !== superName && u.role !== 'super'}
                        ...
                        {#if savedRole === u.username}<span class="ua-flag ua-ok"><Icon name="check" size={16} /> saved</span>{/if}
                      {:else}
              ...
              <div class="ua-actions">
                {#if locked && u.username !== superName}
                  <span class="ua-fixed"><Icon name="lock" size={16} /> admin-tier account — only super admin can manage</span>
                {:else if u.username === superName}
                  <span class="ua-fixed"><Icon name="lock" size={16} /> super admin — fixed account, no changes</span>
                {:else}
                  <button class="ua-btn" disabled={busy === u.username} onclick={() => { resetFor = u.username; rPass = ''; rErr = ''; }}><Icon name="key" size={16} /> Reset password</button>
                  <button class="ua-btn" disabled={busy === u.username} onclick={() => toggleActive(u)}>{u.is_active === false ? ' Enable' : ' Disable'}</button>
                  <button class="ua-btn ua-btn-dangerghost" onclick={() => { delFor = u.username; delConfirm = ''; }}><Icon name="trash" size={16} /> Delete user</button>
                {/if}
              </div>
            </div>
          {/if}
