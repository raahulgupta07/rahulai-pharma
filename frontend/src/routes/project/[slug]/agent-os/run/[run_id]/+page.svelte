...
<div class="page-root">
  <!-- Topbar -->
  <header class="topbar">
    <button class="back-btn" type="button" onclick={backToList} aria-label="Back to workflows"><Icon name="arrow-left" size={16} /></button>
    ...
    {#if status === 'done'}
      {#if dashboardId}
        <button class="topbar-btn primary" type="button" onclick={openFull}><Icon name="trending-up" size={16} /> OPEN FULL</button>
      {/if}
      <button class="topbar-btn" type="button" onclick={openSchedule}><Icon name="calendar" size={16} /> SCHEDULE</button>
      <button class="topbar-btn" type="button" onclick={reRun}><Icon name="refresh" size={16} /> RE-RUN</button>
      ...
      {:else}
        <div class="dash-placeholder">
          ...
          {#if status === 'done' && dashboardId}
            <button class="ph-cta" type="button" onclick={openFull}><Icon name="trending-up" size={16} /> Open full dashboard</button>
          {/if}
        </div>
      {/if}
    </main>
