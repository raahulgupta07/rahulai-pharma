...
<div class="root">
  <header class="bar">
    <div class="left">
      <button class="back" onclick={() => goto(`${base}/project/${slug}/canvas`)}><Icon name="arrow-left" size={16} /> Back</button>
      <input class="name-in" type="text" value={name} oninput={onNameInput} placeholder="Canvas name" />
    </div>
    <div class="right">
      {#if savedFlash}
        <span class="saved"><Icon name="check" size={16} /> saved</span>
      {:else if dirty}
  ...
  {#if err}
    <div class="err"><Icon name="alert-triangle" size={16} /> {err}</div>
  {/if}
      ...
      {#each cards as c (c.id)}
        ...
        >
          <div class="card-head" onpointerdown={(e) => onPointerDown(e, c.id, 'move')}>
            ...
            <button class="x" onclick={() => removeCard(c.id)}><Icon name="x" size={16} /></button>
          </div>
          <div class="card-body">
            {#if c.type === 'note'}
              <textarea
