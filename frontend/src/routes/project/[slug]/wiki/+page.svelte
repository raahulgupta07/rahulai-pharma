...
<div class="wk-root">
  <!-- header -->
  <div class="wk-head">
    <button class="wk-back" onclick={() => goto(`${base}/project/${slug}/overview`)}><Icon name="arrow-left" size={16} /> Dashboard</button>
      ...
      {:else if cur}
        <div class="wk-page">
          ...
          {#if cur.links_out?.length}
            <div class="wk-sec-h">Links <Icon name="arrow-right" size={16} /></div>
          ...
          {#if cur.backlinks?.length}
            <div class="wk-sec-h"><Icon name="arrow-left" size={16} /> Linked from (backlinks)</div>
            <div class="wk-links-list">
              {#each cur.backlinks as l}
                <div class="wk-link-row">
                  ...
                  {/if}
                  <span class="wk-pred">{l.predicate} <Icon name="arrow-right" size={16} /></span>
                </div>
              {/each}
            </div>
          {/if}
          ...
          <div class="wk-ask">
            <button onclick={() => goto(`${base}/project/${slug}?q=${encodeURIComponent('Tell me about ' + cur.name)}`)}>Ask agent about “{cur.name}” <Icon name="arrow-right" size={16} /></button>
          </div>
        </div>
      {:else}
