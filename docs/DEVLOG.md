...
**Fix = FULL OUTER join with an explicit presence column.** Migration **186** (`186_shop_flat_link_status.sql`, applied + ledger + folded into baseline `schema.sql` CREATE/index + seed-marked in `migrations_seed.sql`): new `citypharma.shop_flat.link_status text` ∈ {`both`,`catalog_only`,`stock_only`} + btree index. Backfill: existing rows linked→`both`, else `stock_only`.

- **P4 build rewrite** (`build_shop_flat.py`): keep stock loop (emits `both`+`stock_only`), then add a second loop over catalog keys with NO stock → emit one `catalog_only` row each, **site_code=`'__none__'`** sentinel, qty 0, catalog attrs populated, linked=true. link_status in CREATE+INSERT+return counts. Decision: sentinel site (PK-safe, no row blow-up) over per-(article×site).
- **P5 tools** — new shared `dash/tools/pharma_resolve.py` `resolve_article(cur, schema, query, *, sites=None)` → `{state, art_keys, rows, message}` = SINGLE source of "Not Found" wording. Precedence: any `both`→both, else only catalog_only, else stock_only, else not_found. Bare-code path matches `art_key` exactly so NULL-brand `stock_only` rows are reachable. Catalog presence is **whole-chain** (store-locked key still learns the article exists; only stock QTY store-scoped — `mask_row` untouched). `__none__` excluded from stock listings, included for catalog presence. Wired into `stock_check` (catalog_only → `in_catalog:true`+"in catalog, 0 on hand", NOT "out of stock"), `find_nearby_stock`, `find_substitutes`, `alternatives_for_indication` (empty → explicit "Not Found"). Fail-soft: missing/NULL link_status = `both`.
- **P6 DQ** (`data_quality_scanner.py`): two new table-level checks `catalog_gap` (count catalog_only) + `orphan_stock` (count stock_only), MEDIUM, emit only when >0, fail-soft (to_regclass + column guard). DISTINCT from the existing `sci_notation_id` 1E+12 corruption check — a legit business gap is never reported as corruption.

**Validated end-to-end on a synthetic mismatch** (backed up + restored demo): deleted all stock for 5 articles + inserted 2 orphan codes → rebuilt → `both=1560, catalog_only=5, stock_only=2` (exact reconcile vs catalog−∩). resolve_article: all 4 states correct; precedence correct (Keflex 250mg catalog_only + Keflex 500mg in stock → both, a Keflex IS available). stock_check catalog_only row carries honest `state`/`note`/`in_catalog`. DQ scan surfaced catalog_gap=5, orphan_stock=2. Demo restored to 1600 `both`. Deploy = image rebuild + force-recreate (not hot-copy); leader deleted first. **NOT yet run on the REAL 4600/3800 files** — pending real upload (P1/P2). Plan: `docs/plans/data_upload_pipeline_test.md`.
...
**P3 — `dash/learning/cache_curator.py` + `app/cache_curator_api.py`** (agent B) + **daemon `dash/cron/cache_curator_daemon.py`** + main.py wiring (parent). The "leader decides what to cache" loop. `run_curator(slug, *, dry_run, max_promote=10, days, min_count=3)`: clusters → `_is_cached` skip → **leader judge** (`training_llm_call` framed as lead pharmacy analyst → STRICT JSON `{cacheable, reason, canonical_sql}`; refuses time-relative/personal/per-asker/clinical) → read-only SQL gate (`_is_read_only`: must start SELECT/WITH, no `;`, no DML/DDL) → verify via `verified_reward._run_rows` (must return value) → `_build_card` (valid AnswerCard tags: ACTION_TITLE/KPI 4-field/CONFIDENCE/FRESHNESS) → `await promote_answer(promoted_by='leader')`, cap `max_promote`. `curator_stats`. API (super-admin, auth mirrors usage_api `request.state.user`): `POST /api/projects/{slug}/cache/curate?dry_run=` (default dry True) · `GET .../cache/stats` · `GET .../cache/clusters`. **Daemon DEFAULT OFF** (it WRITES to cache) — opt in `CACHE_CURATOR_ENABLED=1` (also `CACHE_CURATOR_DISABLED=1`, `CACHE_CURATOR_MAX_PROMOTE`), 24h, leader-gated (`_should_run_daemons`), single-tenant `locked_slug()`. main.py: router include after usage_api + lifespan daemon block after chemist_eval (ontology default-OFF pattern).

**LANDMINE found+fixed mid-test:** first curator run → leader **hallucinated table names** (`products`, `inventory_t`) → every SQL `UndefinedTable` → all skipped. Fix = `_schema_context(slug)` injects the REAL schema (information_schema.columns over the project schema → `table(col,…)` lines, capped) into the judge prompt + "use ONLY these tables, never invent; if not answerable set cacheable=false". After fix: 12 valid candidates, "currently"→time-relative refused, 2 promoted as leader rows (real SQL e.g. `SELECT SUM(stock_qty*weighted_cost_price) FROM balance_stock`=500M). 3 HTTP endpoints 200. Transient embed-API hiccup → deterministic fallback (promote still ok). **Test rows then WIPED — prod cache empty (0 rows/0 qcache vectors), daemon OFF so nothing repopulates.** Parallel agents partitioned by file (A=question_clusters.py, B=cache_curator.py+cache_curator_api.py); parent did shared wiring (main.py + daemon file) solo. Deploy: image rebuild + force-recreate, `/api/flags`→200. LOCAL.

---

### Session 2026-06-11 (latest+62) — Answer-cache plan + P0 schema-drift guard on metric_shortcut (LOCAL, LIVE)

**Context.** User asked (research Q): "convert Excel→md/vectors, vectorize everything for fast output?" Answered: **don't vectorize the numbers** — vectors can't count/aggregate; their own traces prove SQL is 13-36ms (not the cost), the LLM round-trip (~6s) is. Best = 3-lane router by question shape (exact→SQL, fuzzy→pgvector row-cards, repeated→pre-computed no-LLM). Named the research (Text-to-SQL/Spider/BIRD, TableRAG, Chain-of-Table, LOTUS). Then designed a **repeated-common-question answer cache**, leader-agent-decided (not hardcoded). Full plan saved `docs/plans/answer_cache_plan.md`. Locked: leader agent promotes (Cache Curator daemon, extends auto_evolve) · semantic embedding NN match (dash_vectors namespace=qcache, cosine ≥0.93) · serve full rendered AnswerCard verbatim. Reuses ~90% existing infra (golden corpus, metric_shortcut serve, verified_reward, golden_drift eviction, embeddings, fingerprints).

**P1 BUILT + LIVE — full-answer semantic cache.** NEW table `dash.dash_answer_cache` (migration 185, applied live + recorded in `public.dash_migrations`): id/project_slug/question/question_norm/canonical_sql/answer_payload(jsonb {content,sql})/source_tables[]/schema_hash/confidence/hit_count/status(live|stale|demoted)/promoted_by/created_at/last_served_at, UNIQUE(project_slug,question_norm). NEW `dash/learning/answer_cache.py` (async): `promote_answer` (stamps source_tables+schema_hash via P0 `schema_guard`, stores the FULL rendered AnswerCard string as `answer_payload.content`, `embed_text`→`dash.dash_vectors namespace='qcache' source_id=cache id`, ON CONFLICT upsert both), `try_answer_cache` (cheap existence check → embed → cosine NN `embedding <=> CAST(:v AS vector)` → `sim ≥ ANSWER_CACHE_MIN_SIM` default 0.93 → schema-drift gate (reuse P0) → serve `content` verbatim, hit_count++/last_served_at), `demote_answer` (status='demoted' + drop vector), `_mark_stale`/`_bump_hit`. WIRED `app/projects.py:~1310` BEFORE metric_shortcut (full answer wins over scalar; only when `model_pref in ('','auto')`), `await`ed (handler is async), stream + non-stream paths, synthetic "Served from answer cache" ReasoningStep trace, bg judge+verified-reward thread. Kill: `ANSWER_CACHE_DISABLED=1`; threshold `ANSWER_CACHE_MIN_SIM`; guard `ANSWER_CACHE_SCHEMA_GUARD=0`. **E2E-verified live (cp-api+real DB, real text-embedding-3-small):** exact serve sim=1.0 hit · close paraphrase "our"↔"the" sim=0.966 HIT · different-topic far sim<thr REJECTED · weak paraphrase 0.637 REJECTED (0.93 = safe, only near-rewords hit — won't serve wrong answer to a subtly different Q) · schema drift → miss + row status=stale · demote → vector rows=0. Reuses HNSW cosine index (no new vector store). Promotion still manual via `promote_answer` (auto-promote = P3). Deploy: image rebuild + force-recreate. Files: `db/migrations/185_answer_cache.sql`, `dash/learning/answer_cache.py`, `app/projects.py`. LOCAL.

**P0 BUILT + LIVE — schema-drift guard on metric_shortcut.** KEY INSIGHT: `try_metric_shortcut` (`verified_reward.py`) re-runs the pinned SQL **live** on every serve → the *number is always fresh*, no stale-value problem. The real risk = the *schema* moving under a pinned SQL (column renamed/retyped/repurposed since pin) → SQL silently returns wrong-semantic with no LLM to catch it (hard breaks already error→fall through). FIX: **NEW `dash/learning/schema_guard.py`** — `sql_source_tables(sql)` (FROM/JOIN regex, CTE aliases harmless), `live_schema_hash(slug,tables)` (combined `dash_table_metadata.col_hash` = cols-only schema fp, "" when no table found = don't gate), `schema_hash_for_sql`. `golden.promote()` stamps `source_tables`+`schema_hash` at pin time (fail-soft). `try_metric_shortcut` carries `schema_hash` through pair-load (now 3-tuple) and, after the acceptance gate, **skips the serve (returns None → agent re-plans) when stored hash ≠ live hash**. Gates ONLY pairs carrying a stored hash (newly-promoted goldens); legacy/auto-QA pairs (no hash, regenerated per-train against current schema) unaffected = no regression. Row-count changes do NOT trip it (live re-run handles those). Kill switch `METRIC_SHORTCUT_SCHEMA_GUARD=0`; guard error = fail-OPEN (never blocks a serve). **E2E-verified live (cp-api + real DB):** promote stamps hash+`['balance_stock']` → fresh serve matched value 1600 → drift col_hash → serve SKIPPED → restore → serves again. Deploy: image rebuild + force-recreate, `/api/flags`→200. Files: `dash/learning/schema_guard.py` (new), `golden.py`, `verified_reward.py`. All LOCAL.
...
**Fixes (all LOCAL):**
1. **Stock fast-path — the big win (`dash/tools/stock_shortcut.py`, NEW).** Pure availability questions ("do we have X / is X in stock / X ရှိလား") answered **with zero LLM** — detect intent (EN + Burmese regex), call `stock_check` directly, format the reply in code. Wired into BOTH `/api/embed/chat` (guard team-build with `if not _shortcut_hit`) and `/api/embed/chat/stream` (emit `step`→`token`→`done` SSE + audit row, then early-return before team build). **Conservative**: `_BAIL_PATTERNS` + `_MULTI` bail to the agent on substitute/clinical/comparative/multi-part wording; 0 matches → fall through; bad extraction → 0 rows → fall through, never a wrong answer. Respects masking (consumer/non-private embeds hide qty+cost — no price leak); store-scoped via the same `API_STORE_SCOPE` contextvar the agent uses; audit-logged as `shortcut/stock` ($0). **Verified live: 13-16s → ~2s** (stream + non-stream); "substitutes for X" correctly falls through (15s, agent).
2. **Reasoning off (`REASONING_FLOOR=0`** in `.env` + compose env block). Counter lookups need no streamed model thinking — the floor was pure ~400-1000ms tax. Tool-step strip still shows (tool events ≠ model thinking). Real analysis (deep pick / ANALYSIS+ tier) still reasons. Also `app/projects.py:1525` now skips the floor for TRIVIAL/LOOKUP tier even when floor on.
3. **Tool discipline (`dash/instructions.py`).** Added a `⚡ ONE-TOOL FAST PATH` rule at top of SHOP COUNTER MODE (call the single matching tool, then STOP; second tool only when the question explicitly asks two things) + scoped the **MANDATORY monograph** to clinical Qs only (was forcing a `drug_profile` call after every `stock_check` to fill `[DRUG:]` tags = the over-fetch). Verified: `stock_check`/`find_nearby_stock` Qs dropped to **1 tool** (was up to 6).
4. **Entry-path trims.** Team cache TTL **60→300s** (`dash/team.py`; staleness still busted by `invalidate_team_cache` on config edits) · training-pairs cache (`app/projects.py` `_load_training_pairs`, mtime-signed — stops re-glob+parse of all training JSON per query) · chitchat scope-skip (greetings never call the classifier LLM).
5. **Embed widget fixes.** Opening starter-chips now render at open (`loadSuggestions()` after `showGreeting` — was only post-first-reply) · `/api/embed/try/` sandbox dropped its hardcoded `data-greeting` so the Burmese config welcome resolves (was English "Hi! Ask me anything") · **gear-pose thinking robot** (`thinkBotSvg()` + `tb-gear` spin/`tb-eye` blink/`tb-ant` pulse) replaces the plain spinner in the live activity strip head.

**Landmine:** the chemist path = the EMBED widget (store-scoped, consumer-mode, concise), NOT `/api/super-chat` (admin BI — full HEADLINE/KPI dashboard cards = verbose = slow). Don't benchmark counter speed on super-chat.

### Session 2026-06-10 (latest+59) — Training-flow timezone/state fix + floating-robot character + RobotAvatar in chat/embed + column-classifier error-proofing (local; tz/state/classifier pushed faa5a2a, robot-anim/callout/avatar LOCAL)

Diagnosed via clocks: training "stuck at elapsed 391 min / LIVE / 0 rows/s" was NOT a hung run — it was **timezone skew**. DB/API run on UTC (`started_at='…15:46:39'`, NO tz suffix); browser is UTC+6:30 → parses the bare string as LOCAL → 6.5h phantom elapsed (only while `finished_at` null). Real run = 4 min. Cascade: stale local `training` flag (backgrounded tab pauses pollers) kept the Train button disabled → "click train, nothing happens."

**1. Timezone + run-state (pushed faa5a2a).** `flow_map.derive_flow` + `learning.py` `/auto-train/status` serialize timestamps as ISO-8601 UTC `…Z` (`_iso_utc` helper). Added `run.phase` ∈ {running,finalizing,done,idle,failed}. **Layer status rewritten** from keyword-guessing `current_step` → MONOTONIC ordered progress from the run `logs` stream (`_LAYER_KEYWORDS` + max-layer-reached; done→all done, live→done-prefix + one running) — fixes "L5–L9 ✓ but L0–L4 blank". Frontend `TrainingFlow.svelte`: UTC-safe `_ts()` parse, 3-state badge `● LIVE / ◐ FINISHING (indexing hint) / ✓ DONE`, rps+ETA suppressed off-running. Verified live: started/finished `…Z`, phase=done, 10/10 done; mid-run `dddddddddr` (monotonic).

**2. Frozen-page self-heal (pushed faa5a2a).** overview + settings re-fetch training status on `visibilitychange`/`focus` (backgrounded-tab fix); Train disabled derives from FRESH backend `is_training`; train errors surfaced (was silent). UTC-safe parse across robot/overview/settings.

**3. Floating robot — character (LOCAL).** `FloatingRobot.svelte`: showRobot now **every screen except chat** (incl. Dashboard); **circle removed → bare animated robot SVG** + ground shadow; idle bob/blink/look-around; **training → hard-hat 👷 + spinning gear + indeterminate progress ring**; **done → confetti+✓+bounce**, **error → red+⚠+sweat+shake**, **paused → sleeping+Zzz**; **task-specific props** (indexing=🔍, embedding=orbiting dots, eval=clipboard✓) from new backend `task` field; **attention badge**, **drag-to-reposition** (localStorage `fr_pos`), **poke wiggle**; `prefers-reduced-motion` respected. Backend `/auto-train/status` gained `task` (training/indexing/embedding/eval/idle/paused from current_step) + `attention` (failed-run count) + `callout`.

**4. Robot callout catalog (LOCAL).** Client-side rich `callout` in FloatingRobot: `STEP_MSG` map (current_step→friendly line: qa_generation→"Writing practice questions ❓", knowledge_graph→"Mapping the drug network 🕸️", finaliz→"Filing everything away 🗂️", embed→"Memorizing with embeddings 🧠"…) + rotating variant pools (idle/train/done/error/paused) rotated every ~12s via `nowTs`; priority poke>done>error>training-step>paused>idle; hover wave line.

**5. RobotAvatar in chat + embed (LOCAL).** New shared `frontend/src/lib/RobotAvatar.svelte` (mini robot, moods idle/thinking/typing/tool/done/error, pure SVG+CSS). `ChatMessageList.svelte`: swapped ONLY the message avatar box + the 3 "generating…" `▪▪▪` row icons → `<RobotAvatar>` (mood from `msg.status`: streaming+empty=thinking, streaming+content=typing, error=error, else done). **Trace card / steps / layout untouched** (per user: "keep this part, just add robot"). `widget.js` `avatarContent()` → same robot SVG (white-on-accent circle + antenna pulse) for embed header + per-message; logo override still wins. Verified `bot-robot` baked into served widget.js.

**6. Column role classifier — error-proof (pushed faa5a2a).** Bug: `_sql_profile_columns` (app/upload.py) classified `distinct ≤ 500 → dimension` FIRST, stealing numeric measures (base_price) + unique keys (article_code) before measure/id checks → catalog table profiled `7 dim · 0 measure · 0 id`, breaking price Q&A. FIX: reorder — id-name (`*_code/_id/_key`)+uniqueness → high-cardinality unique → numeric→measure (BEFORE low-card) → dimension → text. Mirrored id-name rule into `profile_v2._classify_role`. Verified live full retrain: articles_clean now `4 dim · 1 measure · 2 id`, `base_price[350–5000]`=measure, article_code=id. NOTE: the 5–6 articles_clean Q&A SQL errors are a SEPARATE bug (Q&A-gen scoping — references stock cols absent in the catalog table), NOT the misclassification; still open.

Workspace Cockpit: removed the duplicate 0%-bar + KG/Vector-Backfill pill strip (robot+dashboard already show training). Robot tz/state/classifier committed `faa5a2a`; robot animations + callout catalog + RobotAvatar are LOCAL/uncommitted.

### Session 2026-06-10 (latest+58) — Deployments page: widget lifecycle + auth modes + ConfirmModal + widget streaming/Burmese (local)

All deployed (rebuild + `--force-recreate`) + verified live. NOT pushed.

**1. "Widgets" → "Deployments".** Renamed embed nav label + page title (`EmbedPanel.svelte`); kept route id `#widgets`.

**2. Widget lifecycle controls.** Two sub-tabs: **🏪 Outlet** (auto-provisioned, `store-*`/`bound_scope_id`, PERMANENT) vs **✦ Custom** (user-built). Per-row **enable/disable** sliding toggle (PATCH `{enabled,status}`). **Delete** = custom-only, double-confirm, hard-delete; outlet rows show 🔒 Locked. Backend DELETE guard in `app/embed.py` → 403 for store/`bound_scope_id` embeds (API can't drop an outlet). Status badge `● Live`/`◌ Paused`. Row dims when off. ACTIONS cluster (`snippet/.zip/test/config`) mirrors detail footer. Fixed 2 latent `revokeEmbed` ReferenceErrors (fn was removed earlier) in detail-footer + cockpit.

**3. Auth modes public/hmac/jwt.** Per-row `<select>` (instant PATCH). Global **default** card on page → setting `embed_default_auth_mode` (enum, default public; `dash/admin/settings.py`) read by `manager.py` auto-provision. **Bulk-apply** button → `POST /{slug}/embeds/bulk-auth` (loops every widget). New endpoints: GET/PUT `/{slug}/embed-default-auth`, POST `/{slug}/embeds/bulk-auth`. Verified: GET/PUT round-trip, bad=400, bulk=66 updated/0 failed. public=drop-in (no backend), hmac=server-signed (PHP), jwt=app identity; admin-only.

**4. `ConfirmModal.svelte` (new).** Branded warm modal (blur backdrop, Esc/click-out/Enter, `typeToConfirm` input disables confirm until match, `danger` red, `hideCancel` info-mode = single OK). Replaced EVERY native `confirm()`/`prompt()`/`alert()` in the panel (bulk-auth, delete, rotate-key, result/error notices) via `askConfirm()`/`notify()` + one top-level `confirmModal` state. LANDMINE: keep position:fixed overlays at template top level, not inside a route `{#if}`.

**5. Embed widget — token-by-token streaming (consumer mode).** `app/embed_public.py`: replaced buffer-once-emit-once with incremental safe-streamer (`emitted_len` cursor + `_consumer_hold_len()` hold-window + `_sanitize_fragment()` no-truncate variant; flush full `sanitize_consumer_response` at end). Holds tail that could be open `[`/```` ``` ````/`<code`/partial table. Verified E2E: 18 token frames (was 1), 0 leaks. Security-sensitive: holds when ambiguous.

**6. Burmese openers.** Migration **184** `dash_agent_embeds.starter_questions JSONB`. Settings `embed_default_welcome` (`မင်္ဂလာပါ — ဘာများ ကူညီပေးရမလဲ?`) + `embed_default_starters` (3 Burmese pharma chips: လက်ကျန်/အစားထိုး/အနီးဆုံးဆိုင်). `/config/{id}` resolves welcome (per-widget??brand??setting??Burmese) + returns starters; `/config/{id}/suggestions` returns starters; `widget.js` renders chips from config. `_BRAND_HARD["welcome_msg"]`→Burmese, auto-provision stops baking English; 66 English greetings nulled. Verified live.

**7. Speed.** Team-cache reuse confirmed (per-store, not per-msg). No safe fast-tier knob at embed call-site — recommend optional `forced_tier=lite` in `create_project_team` as a deliberate follow-up (NOT done; streaming masks latency anyway).

**OPEN finding (pre-existing, not introduced):** consumer outlet embed (`bound_intent=public`) test reply showed qty + cost — price/qty masking not applied for that intent. Streaming faithfully emits the same sanitized text → leaks in blocking path too. Needs consumer price/qty lockdown.

Build ran via 2 parallel agents (Part A modal in `EmbedPanel.svelte`/`ConfirmModal.svelte`; Part B streaming+Burmese in `embed_public.py`/`widget.js`/migration/settings) — file-disjoint, no conflicts; parent rebuilt image once after both.

### Session 2026-06-10 (latest+57) — Training-flow log cleanup + flow OBFUSCATION (anti-reverse-engineer) (local)

Three follow-ups on the live training flow (`TrainingFlow.svelte` + `flow_map.py`), all deployed (rebuild + `--force-recreate`) + verified live. NOT pushed.

**1. Step-value multi-line leak (`flow_map.py` `_parse`).** Per-step `value` was leaking a whole multi-line log dump (`[SQL:\n11:43:57 ✓ shop_flat: 1600 rows\n…`) into one step box. Fix: take FIRST non-empty line only + drop bracketed section markers (`[SQL:`, `[CONFIDENCE…]`). Verified 0 leak rows on live endpoint.

**2. Removed duplicate dark CLI log panel (`TrainingFlow.svelte`).** Right rail had a `.tf-log` dark console mirroring the run log — redundant with the robot-agent live log. Deleted markup + `fetchLog()`/`logEvents`/`logSince`/`logEl` state + `/auto-train/log` poll in `tick()` + legend note + CSS. Rail now = DATA STORES only. (`auto-train/log` endpoint untouched — robot agent still uses it.)

**3. FLOW OBFUSCATION — hide internal tool/engine/table names so the flow can't be reverse-engineered (default ON).** Flag `FLOW_OBFUSCATE` (env, default `1`; set `0` for internal debugging = real names). Masks the WHOLE training-flow surface so even the raw API response (network tab) carries no internal identifiers:
- `flow_map.py`: `_MASK_STEPS` (60 real step labels → generic function names: `Engineer.inspect_schema`→`probe_structure`, `knowledge_graph`→`build_network`, `shop_flat build`→`denorm_build`, `qa_generation`→`gen_pairs`, `build_ddl + CREATE`→`materialize`…), `_MASK_TITLES` (10 layer titles → `STAGE N · …`, drops "Engineer agent"/"Apache AGE"/`dash/ingest/`), masked `detail` + generic `writes_to`. **Internal labels still used for log-matching, then masked at serve time** (`_mask_flow` in `derive_flow` just before return). `_scrub()` cleans DYNAMIC log-derived `value` + `run.current_step`/`progress.step|table`: `_LABEL_SCRUB` (identifier labels appearing verbatim in logs e.g. "step knowledge_graph: ran 4910ms") + `_SCRUB` token list (Apache AGE→graph store, PgVector→vector index, matview→managed view, `citypharma.`/`dash_*`/`shop_flat`/`articles_*` stripped, postgres→primary store, openCypher→graph).
- `trainingFlowSpec.ts`: fallback `FLOW_LAYERS` (60 steps) + 10 titles + 11 `STORES` masked IDENTICALLY via one-shot script importing the same `_MASK_STEPS`/`_MASK_TITLES`/`_RAW_PHASES` (so pre-fetch flash + offline matches live). Rail store labels → generic (Graph store, Vector index, Primary store, Knowledge base…).
- `TrainingFlow.svelte` `writeKey`: now routes the MASKED generic targets (`(intake)`,`Vector index`,`Graph store`,`managed view`,…) → store keys so step `→ writesTo` click-through to the store modal still works; legacy real targets kept for `FLOW_OBFUSCATE=0`.
- `learning.py` store-detail modal: `_SMASK` (11 keys) replaces label/table/blurb with generic when obfuscate ON (imports `flow_map._OBFUSCATE`); your actual data rows/columns still shown (that's customer data, not IP).
- Verified live: 0 internal-name leaks across titles/labels/details/writesTo/values/run-progress + all store-detail blurbs generic. (Only `vector_backfill` shows — a deliberately-kept generic masked label, not a tool/engine/table name.)

### Session 2026-06-10 (latest+56) — Demo seed + Data Source origins/expand + clickable stores + live per-step training trace + catalog passthrough (local)

Multi-feature Data Source / Dashboard session. All deployed (rebuild + `--force-recreate`) + verified live. Mostly NOT pushed (origins + demo seed pushed at `c34abd9` on `rahulai-pharma`; rest local).

**1. Table origin + expand-all (Data Source).** `app/datasource_api.py` `_classify_origin(name, meta, cols)` → `uploaded|ai|derived` (order: `metadata.origin` override → `_DERIVED_NAMES={shop_flat}`→derived → `catalog_enrichment` or `{model,suggested_value}⊆cols`→ai → else uploaded). `/datasource` adds `origin` per table + `summary.origins`. Heuristic-only (no write-backs) — covers current+future known tables, zero training-path risk. `settings/+page.svelte`: ORIGIN badge column (📤/🤖/⚙️) + 3-bucket origins summary card + **expand-all default** (Data Source list `dsExpanded:str|null`→`dsOpen` map `!==false`; Exploratory `edaColKey`→`edaColOpen`; ⊕/⊖ toggles). Verified: catalog_enrichment=ai, articles_clean/balance_stock=uploaded, shop_flat=derived.

**2. Synthetic pharma DEMO SEED (auto + button).** `scripts/seed_pharma_demo.py` — `generate_demo_dataframes()` (pandas/numpy, no faker; 62 real generics → SKU per brand×strength, 54 multi-brand so substitutes work; `article_code` TEXT 8-digit; 8 Myanmar stores; ~14% OOS) → `articles_clean`(200) + `balance_stock`(1600), cols match CATALOG/STOCK resolvers + build_shop_flat. `seed_demo(slug,force,train)`: `_project_is_empty` guard, `_ensure_project_row` (locked `dash_projects` row — fixes fresh-AWS 404), brain seeds, `_trigger_training` (bg thread mints super-token→INSERT `dash_tokens`→urllib self-POST `/retrain?force=1`, waits for server up). KEY INSIGHT: retrain discovers tables via `get_table_names(schema)` → seed = to_sql + retrain, no training dup. **Boot hook** `app/main.py` lifespan after rls_blueprints, `WORKER_RANK==0` + `DEMO_SEED_ON_EMPTY=1` default-on → `seed_demo(force=False,train=True)`. **Endpoint** `POST /api/projects/{slug}/seed-demo?force=1` (editor) → `seed_demo(train=False)`, frontend kicks retrain w/ user token for live console. **Button** `⚗ Load demo data` (`dsSeedDemo`) next to ＋Upload, confirm + force-replace if data exists. CLI `python scripts/seed_pharma_demo.py [--force] [--no-train]`.

**3. LIVE DB data swap (this install).** User: "remove real data, add demo in docker." Backup `pg_dump -n citypharma -Fc` → `./backups/pre_demo_wipe_20260610_180052.dump` (6.6M, reversible). Dropped 4 tables + cleared slug-scoped artifacts (dash_table_metadata/training_qa/training_runs/dash.dash_vectors/knowledge_triples = 11/164/11/4984/14). `seed_demo(force=True,train=True)` → 1800 rows, full 14-step train (~9min), shop_flat 1600 (join clean). Chat verified: "200 distinct SKUs" exact. Restore: `docker cp ./backups/<dump> cp-db:/tmp/r.dump && docker exec cp-db pg_restore -U ai -d ai --clean --if-exists -n citypharma /tmp/r.dump`.

**4. Serpentine pipeline schematic (`lib/TrainingFlow.svelte`).** Was 1-row `overflow-x:auto`. Now CSS grid `repeat(5,…)` 2 rows; `gridRow`/`gridCol` (odd rows reversed = continuous snake); directional arrow connectors (→ row1, ↓ U-turn under L4, ← row2) drawn into gaps via pseudo-elements + `tf-sdash`/`tf-sdashv` keyframes; done=solid, running=animated.

**5. Clickable DATA STORES → detail modal.** `GET /api/projects/{slug}/store-detail/{key}` (`app/learning.py`, `_STORE_SPEC` 11 keys → blurb+columns+top-20 SQL+count, all fail-soft). Rail rows → `<button>` → modal (header icon·table·count, blurb, live sample table, Esc/backdrop close). Verified all 11 (PG 3, QA 22, BRAIN 160, VEC 135, AGE 47…; ENR empty→graceful note).

**6. Live per-step training trace.** Was layer-coarse (all steps in a "done" layer = ✓) + static spec text. `dash/training/flow_map.py` `derive_flow` now enriches each step `{state, value, ms, cost}` by reconciling spec label against run `logs` (already embed real values) + `dash_training_steps` (tail timing) + `current_step`; layer headers get `step_done/step_total · elapsed · $cost`. No migration, no engine refactor. `TrainingFlow.svelte` body renders live (icons ✓/◉/✗/⚠/⊘, real value, ms, layer badge) + `→ writesTo` clickable→store modal + `OFF` chip on gated. Done-layer running→done downgrade; tree-char strip. Verified: `profile_v2: 4 cols · scan=11ms`, `deep_analysis: gemini… 14.03s`, L3 `12/12 · 18.9s · $0.0078`, `evals: 5/10 passed`, skipped `goals`→⚠.

**7. catalog_apply passthrough fix.** `build_articles_enriched_view` (`app/catalog_apply.py`) precheck `to_regclass`: source absent→skip; `catalog_enrichment` absent (enrichment OFF) or full view fails→**passthrough** `CREATE OR REPLACE VIEW articles_enriched AS SELECT * FROM articles_clean`. Kills red "view build failed: UndefinedTable" on lean/demo catalogs; `articles_enriched` always exists for shop_flat. Verified passthrough → 200 rows, no error.
