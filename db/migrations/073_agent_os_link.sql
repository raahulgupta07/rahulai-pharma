-- 073_agent_os_link.sql
-- Wire all Dash agents into Agent OS hub: extend registry columns, seed all
-- chat/specialist/background/upload/extended/dream/autosim/sim/janitor/learning
-- agents, and add 2 missing AutoSim cron schedules.
--
-- Schema reality (verified via \d):
--   public.dash_agent_registry(id BIGSERIAL PK, agent_name TEXT UNIQUE,
--                              description TEXT, tags JSONB, created_at,
--                              category TEXT)
-- Natural key for upsert = agent_name (kebab-case ids treated as agent_name).
-- All extra columns are added defensively below.
