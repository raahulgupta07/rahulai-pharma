...
### Fixed
- **Catalog-query latency 10s → ~0.02s + count correctness (2026-06-17, v1.49.2)** — users reported catalog questions were slow (7-11s). Two causes: (a) `store_stock_summary` rejected the `site_code` arg the model always passes (per SHOP CONTEXT rule) → `ValidationError` → wasted retry turn; fixed by accepting + ignoring `site_code` (branch already auto-binds). (b) "how many products", "list/which medicines with X" hit no 0-LLM fast-path → full analyst. Extended `dash/tools/stock_shortcut.py` with deterministic **count** (→ `store_stock_summary().unique_articles`) and **browse-by-ingredient** (→ `stock_check(term)`) intents. Result: "how many products" 10.7s→28ms and now correctly **1,721** (model had mis-counted 30 top-rows); "medicines with amoxicillin/paracetamol" 7-10s→~20ms with real rows. Guards verified: availability still fast-paths, clinical/symptom/multi-intent still fall to the agent (no wrong answers).
- **Tool telemetry restored (2026-06-17, v1.49.1)** — the per-call tool recorder (`dash.dash_tool_utility_scores`, feeds dashboards + skill-refinery) was silently dead: migration `085` step-2 (per-call columns) was never applied on this DB → every flush threw `UndefinedColumn` → tool failures (incl. the `search_all not found` + not_found loops behind the v1.49.0 bugs) were invisible. Applied `085` (idempotent ALTER, +10 nullable cols). Also fixed a latent classifier bug in `skill_refinery._classify_result`: `str.startswith("")` is always True, so EVERY string tool-result was logged as a failure (100% fake-fail; skill-refinery would penalise healthy tools). Now: dict `ok:false` = failure, `not_found`/normal = success. Verified rows land with correct success/latency.

### Added
- **Dynamic / self-adapting schema (2026-06-17, v1.51.0, flag `ADAPTIVE_SCHEMA`)** — uploads whose columns changed no longer break ingestion, `shop_flat`, or chat; the pipeline adapts and records the change, with a review gate for ambiguous renames. Six phases (plan `docs/plans/dynamic_schema.md`, harness `scripts/verify_dynamic_schema.py` 12/12): **(1)** audit + registry tables (migration `194`: `dash_schema_events`, `dash_column_map`) + fail-soft helpers (`dash/ingest/schema_events.py`); **(2)** adaptive ingest in `ingest_promote` — on drift: snapshot `{tbl}__bak`, add new cols, NULL-fill removed, widen retyped to TEXT, evolve contract (default OFF = legacy quarantine); **(3)** logical→physical column registry + resolver (`dash/ingest/colmap.py`) so stable logical names (`brand`,`cost`) survive header renames; **(4)** `build_shop_flat` resolves columns through the registry with a no-empty guard (keeps last-good table if a required column is missing); **(5)** rename matcher (auto-map ≥0.85, else pending) + review UI (`ColumnMapReview.svelte` at Command Center → Data → Column Mapping) + endpoints `GET/POST /ingest/{p}/colmap`, `/colmap/decide`, `GET /ingest/{p}/schema-events`; **(6)** auto-retrain + schema-change feed on adapt/confirm. Also ships **empty-load guards**: a 0-row parse is refused (clear error, prior data kept) instead of silently creating an empty "trained 0%" table that retrain can't fix. Landmine recorded: `public.dash_*` writes need `get_write_engine()` (the guarded `get_sql_engine()` blocks public-schema writes). Embed admin also gains a **global Settings** model selector (Dash 3.0/2.0/1.0) + header button-wrap fix.
- **Dash 3.0 in the main app — badge + dropdown (2026-06-17, v1.50.1)** — extends the v1.50.0 speed tier from the embed widget to the main app chat. Per-answer badge: every zero-LLM serve in the project chat (`app/projects.py`) and global super_chat (`app/main.py`) now emits a `RouterDecision {engine:'3.0'}` event (answer-cache, verified-metric, query-bank, proven-shortcut), so the answer meta shows **Dash 3.0** (`ChatMessageList.svelte` now 3-way). Composer dropdown (`project/[slug]/+page.svelte`) gains **Dash 3.0 · Auto** as the new default ("Instant for simple asks — scales up only when needed"); selecting it maps to auto routing (`engine '3.0' → ''`). Verified live on project chat: "how many products" → engine 3.0, complex analytical Q → engine 1.0. Super_chat has no engine form-pref + homepage has no dropdown (left as-is).
- **Dash 3.0 / 2.0 / 1.0 speed badge in the widget (2026-06-17, v1.50.0)** — each answer now shows which engine served it, on the trace head next to "Thought for Ns": **Dash 3.0 · Instant** (0-LLM fast-path — count/stock/browse, ~30ms), **Dash 2.0 · Fast** (single analyst), **Dash 1.0 · Pro** (full team). Truthful per-answer badge (reflects the actual routing, not a user choice — a complex question can't be forced onto the fast-path). Backend tags `engine` on every `done` event in the embed stream (`app/embed_public.py`); widget renders the pill in `onDone` (`dash/embed/widget.js`). Verified: "how many products" → 3.0/36ms, "is biogesic in stock" → 3.0/30ms, "what is paracetamol used for" → 2.0/8s.
- **Embed chat grounding + `top_products` tool (2026-06-17, v1.49.0)** — fixes wrong-field + hallucination on the store widget (RCA `docs/RCA_embed_chat_2026-06-16.md`). New `store_stock_summary(top_products=true, category=)` returns the real highest-stock SKUs (per branch, optional category) from `shop_flat` — the missing capability that drove the model to invent product lists. Four grounding rules added to SHOP COUNTER MODE (`dash/instructions.py`): **P5** no-fabrication on empty/`not_found` (say "no data", never invent rows), **P6** SKU-vs-units field disambiguation (`unique_articles` != `total_stock_qty`), **P7** stop-on-empty (no retry loops); plus `search_all` no longer mandated in store-locked mode (was erroring `Function search_all not found`). Verified live: "how many SKUs" → 1,721 (was 24,971 units); "top products in this category" → asks which, then real branch rows (was fabricated AMODEPIN/METFORMIN).
