...
def render(step, status):
    ...
    mark = {"done":f"{GREEN}OK{RESET}","run":"{spin}","todo":f"{GREY}·{RESET}"}[status]
...
def run():
    ...
    print(f"{BOLD}{GREEN}OK pipeline complete{RESET} {DIM}status=done · 3 matviews · gaps reviewable · eval 86.7%{RESET}\n")
