...
- **Like / Dislike** (Learning, 2026-06-10) — 👍/👎 totals + **satisfaction %**, by-project satisfaction, and **top-disliked answers** (question + answer + SQL = retrain candidates). From `dash_feedback` (`GET /api/admin/usage/feedback`). *(Per-model satisfaction is a known gap — `dash_feedback` has no model column.)*
