...
**Chat presentation — Claude.ai / Aria clean (2026-06-15):** the analyst chat renders in a **`claude`** look by default, rebuilt to match Claude.ai and the sibling Aria app. Empty state = centered sparkle greeting + composer-as-hero + suggestion pills + a quiet stats line (`N datasets · N rows · N sessions`). Each answer is a clean flow: a **boxless gray reasoning line** (a one-sentence summary of what the agent did, e.g. *"Planned with analyst, searched internal knowledge, queried the database"*, with a trailing `›` you click to expand the step timeline — humanized step titles, no raw tool names, foldable SQL/call code) → **serif, box-free answer prose** → **SOURCES** chips (the datasets the answer drew from) → a quiet meta line (**`Dash 1.0 · Auto · N tok · $cost · time`**) → an **EXPLORE** row of follow-up question chips → a one-line *"Saved N to memory"* you can expand to see the learned facts. The model is branded **Dash 1.0** throughout (a UI name — the underlying router/model is an implementation detail). The composer is a single rounded card: input on top, a segmented **Auto · Quick · Deep** speed toggle bottom-left, the model name + a send button bottom-right. The whole column is width-matched (880px) so the composer lines up with the answers.
