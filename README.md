...
**Look & feel (2026-06-13):** soft rounded corners driven by 4 CSS knobs in `frontend/src/app.css` (`--pw-radius` 12px / `-sm` 8px / `-button` 8px / `-pill` 999px) — re-tune all roundness from there. Nav pills are rounded rectangles (not stadium). Dashboard runs full window width. Top nav: **Chat · Dashboard · Agent Brain · Endpoints · Admin** — "Agent Brain" is the project Settings page (data + the Brain knowledge hub via its left rail); there is no separate top-nav Brain (one hub, one door). **Top-bar declutter (2026-06-15):** the account chip is one tight inline row (avatar · name · role · ⌄); the notifications control is a bell icon with a floating unread count (no "Feed" label); the old in-chat drift bell was removed (drift alerts live in admin/dashboard). **Embed widget — Claude.ai look (2026-06-15):** the drop-in `dash/embed/widget.js` now matches the main chat — assistant replies render bare (no chat bubble), the "thinking" trace is a quiet collapsible `Thought for Ns ›` line in past tense (no green "OK" chips), a live elapsed timer ticks while it works, the header is slim (`name 🟢 Online`), the per-message bot avatar is gone, and the chat-home recommendation chips appear instantly (defaults first, enriched in parallel) instead of stalling on serial table inspections.

**Icons, not emoji (2026-06-14):** the UI uses one in-repo SVG icon component — `<Icon name="…" size={16} />` from `frontend/src/lib/Icon.svelte` (Lucide-style inline SVG, `stroke=currentColor`, ~135 named glyphs). The whole codebase was swept emoji-free; add a new icon to that path map rather than dropping a colorful emoji into markup. (Backend log/label strings use plain ASCII for arrows/checks — `→`→`>`, `✓`→`OK` — and no pictographs.) Button labels carry their glyph as a leading `<Icon>` in markup, never as a `<`/`>` character inside a `{ }` string (that renders the HTML entity as literal text) — the overview toolbar (Upload / Sync / Auto / Refresh / Train) follows this.

**Chat presentation — Claude.ai / Aria clean (2026-06-15):** the analyst chat renders in a **`claude`** look by default, rebuilt to match Claude.ai and the sibling Aria app. Empty state = centered sparkle greeting + composer-as-hero + suggestion pills + a quiet stats line (`N datasets · N rows · N sessions`). Each answer is a clean flow: a **boxless gray reasoning line** (a one-sentence summary of what the agent did, e.g. *"Planned with analyst, searched internal knowledge, queried the database"*, with a trailing `›` you click to expand the step timeline — humanized step titles, no raw tool names, foldable SQL/call code) → **serif, box-free answer prose** → **SOURCES** chips (the datasets the answer drew from) → a quiet meta line (**`Dash 1.0 · Auto · N tok · $cost · time`**) → an **EXPLORE** row of follow-up question chips → a one-line *"Saved N to memory"* you can expand to see the learned facts. The composer is a single rounded card: input on top, a segmented **Auto · Quick · Deep** speed toggle bottom-left, an **engine dropdown** + a send button bottom-right. **Engine selector + truthful speed badge (2026-06-16; Dash 3.0 added 2026-06-17):** the old static "Dash 1.0" label is now a three-tier picker (higher number = faster) — **✨ Dash 3.0 (Auto, default)** = smart routing that answers instantly when it can (a deterministic 0-LLM fast-path for stock checks / product counts / ingredient browse, ~30ms) and scales up only when the question needs it; **⚡ Dash 2.0 (Fast)** = a single fast Analyst for simple lookups (~10s); **◆ Dash 1.0 (Pro)** = the full multi-agent team for the deepest analysis. The choice persists per browser. It's a *preference, not a hard switch* — a complex/analytical question auto-uses the team even on a faster tier, so quality is never traded for speed (both return identical numbers). The per-answer meta line shows a **truthful badge of which tier actually served the answer** (Dash 3.0 Instant / 2.0 Fast / 1.0 Pro) — driven by a `RouterDecision` event from the backend (zero-LLM serves tag `engine:'3.0'`), so a complex answer can never masquerade as instant. The same badge appears in the embed widget next to "Thought for Ns". The whole column is width-matched (880px) so the composer lines up with the answers.

**Chat speed (2026-06-16).** Cold chat was 20-60s — the sum of ~10-15 serial LLM round-trips from multi-agent coordination. Three fixes cut that: (1) a **single-agent fast-path** (`SINGLE_AGENT_FASTPATH`, the engine behind Dash 2.0) runs a lone Analyst on the simplest TRIVIAL/LOOKUP questions instead of the full leader↔member team — cold LOOKUP **32-63s → 8-18s**, with anything heavier still routed to the team; (2) an **RLS search_path fix** that eliminated 102 caught DB exceptions per request (the row-level-security engines were querying a `dash`-schema table on the wrong search_path, erroring on every SQL statement then falling back to passthrough); (3) **rerank fail-fast** (a cohere rerank that could hang 21s now hard-caps at ~1s) + a smaller per-turn history window (`NUM_HISTORY_RUNS`). All env-tunable and reversible. **Embed engine** is selectable too — a global default plus a per-widget override in the Embed admin (`embed_default_engine` + each widget's `feature_config.engine`).
...
**Embed security & cross-origin (2026-06-15).** Each embed key carries its own `allowed_origins` allowlist (empty = block-all, the secure default; modes `strict|subdomains|open`, glob `https://*.acme.com` supported) — a request from a non-listed site gets `403 origin_denied`. CORS is enforced as a **single consolidated policy**: the app strips the extra `CORSMiddleware` that Agno AgentOS injects and re-adds exactly one driven by `CORS_ORIGINS`/`PUBLIC_URL` (never the browser-rejected `Allow-Origin: *` + credentials combo; the widget itself fetches with `credentials:'omit'` so `*` is safe for it). The sandbox/snippet URLs honor `X-Forwarded-Proto` so they emit `https` behind a tunnel/ALB (no mixed-content block). **Testing:** `scripts/embed_smoke.sh <BASE> <embed_id> <pub_key> <origin>` validates the whole chain headless; `scripts/set_embed_origins.sh` manages a key's allowlist; `scripts/embed-test.html` is a real cross-origin browser harness. To expose a local instance publicly without cloud hosting, a Cloudflare quick tunnel (`cloudflared tunnel --url http://127.0.0.1:8011`) gives a free HTTPS URL — verified end-to-end with a real third-party origin embedding the widget.
...
- **Robustness (2026-06-13):** a **per-table timeout** (`RETRAIN_TABLE_TIMEOUT_S`, default 600s) is the hard ceiling — one hung table/step can no longer wedge the whole batch (the old `as_completed` loop blocked forever → run went stale at 12 min with empty logs); on timeout it logs `⊘ table X timed out`, counts it failed, and continues, and the pool shuts down without joining the orphaned worker. A **startup heartbeat** (`▶ retrain started — N table(s)`) means a run's `logs` is never empty, so a post-mortem can always see where it stuck. The watchdog now also stamps the `error` column (was only the invisible `current_step`), `/auto-train/status` returns `error`+`current_step`, and the floating robot console shows `⚠ last run failed: …`. A **circuit-breaker** in the auto-train daemon (`AUTO_TRAIN_MAX_RETRY`, default 2) stops the daemon re-triggering the same never-completing table-set forever (`get_daemon_status().circuit_open` lists any paused set); it clears when the set changes or trains clean. The robot's ⚠ alert badge no longer latches forever — it clears on dismiss (✕), on a newer run, or after 6h idle.
- **Watchdog false-abort fix (2026-06-14):** the watchdog measures liveness from the newest `logs` heartbeat only, so a few long-but-quiet finalize steps (catalog-vector embedding, knowledge-graph build, scope derivation) could go silent past 12 min and get killed *while still working*. Those three steps now emit a per-batch/per-milestone heartbeat into `logs`, so a healthy run stays visibly alive (smoke test: max gap between log lines dropped from >12 min to **73 s**). A genuinely stuck step still emits nothing → the watchdog correctly fails and auto-retries it. **Any new long-running step must emit a `logs` heartbeat or it will be false-aborted.**

### Data-safety guards on replace / S3 sync (2026-06-13)
A `replace` that overwrites a live table is destructive (a `DROP … CASCADE` then reload). Unattended S3 sync (`app/s3_sync.py`) now POSTs `/api/upload` with `guarded=1`, which turns on a set of holds so a bad automated export can't silently corrupt a table — **manual UI uploads stay flexible (guarded off)**:
- **Empty file** → `400`, existing data kept (a header-only/0-row export never blanks the table; the sync's `≥400 → continue` skips the etag write so a corrected file re-syncs).
- **Schema drift** — incoming file DROPs/renames an existing column → `409 {error:"schema_drift", removed_columns:[…]}` held. Added columns are allowed.
- **Row-count cliff** — incoming rows `< REPLACE_MIN_ROW_PCT%` (default 50) of current → `409 {error:"row_count_cliff"}` held (catches partial exports). First-ever load always allowed.
- **Pre-replace backup** — before the irreversible drop, the current table is snapshotted to `"{tbl}__bak"` (kept ×1, skipped above `REPLACE_BACKUP_MAX_ROWS`, default 2M). Restore with `POST /api/projects/{slug}/restore-table` `{"table":"<name>"}` (editor-gated). Backup tables are excluded from training.
- **Notifications** — a sync error or hold drops a row into `dash_notifications` for every admin/super (the bell), so a failing/disabled source isn't silent.
...
### Dynamic / self-adapting schema (2026-06-17, flag `ADAPTIVE_SCHEMA`)
A new upload whose **columns changed** no longer breaks ingestion, `shop_flat`, or chat — it adapts and records the change; ambiguous renames go to a one-click review gate. (Plan: `docs/plans/dynamic_schema.md`.)
- **Adaptive ingest** — on schema drift the pipeline ADAPTS instead of quarantining: new columns auto-added, removed columns NULL-filled (never dropped), changed types widened to TEXT, and a `{tbl}__bak` snapshot is taken first. `ADAPTIVE_SCHEMA` default-OFF reverts to the legacy quarantine.
- **Logical column registry** (`public.dash_column_map`) — a stable *logical* name (`brand`, `cost`) maps to whatever the *physical* column is called (`brand_name`, `weighted_cost_price`). `shop_flat` and the tools read logical names, so renaming a raw header doesn't touch chat. `build_shop_flat` resolves through the registry and **keeps the last-good table** rather than rebuilding empty if a required column is missing.
- **Rename review gate** — an obvious rename auto-maps (high name similarity); an ambiguous one is parked as *pending* and surfaced at **Command Center → Data → Column Mapping** with Confirm / Reject / Map-to, alongside a **Schema Changes** feed. Confirming rebuilds `shop_flat` + retrains.
- **Empty-load guard** — a file that parses to **0 data rows** is refused (clear error, previous data kept) instead of silently creating an empty "trained 0%" table that retrain can't fix.
...
- **Preview before committing** — the OKF lane (`use_okf`) is read by chat **on by default (2026-06-14)**; the in-chat toggle pill was removed to keep the composer clean. With no bundle imported the pending lane is empty, so behaviour is identical to live; once you import a `.zip` the answers can draw on that pending knowledge before you promote it. (Override per-browser via `localStorage cp_use_okf='0'`.)
...
- **Deployments admin list** (`#widgets`, rebuilt 2026-06-10): **two sub-tabs** split widgets by origin — **🏪 Outlet** (auto-provisioned per DB store: `store-*` / `bound_scope_id` set; **permanent**, can't be deleted) vs **✦ Custom** (user-built; `+ New widget` lives here). Each row has an **enable/disable** sliding toggle (`● Live`/`◌ Paused`, PATCH `{enabled,status}`; row dims when off), an **ACTIONS** cluster (**⧉ snippet · ⬇ .zip · ▶ test · ⚙ config**), and a **manage** slot: **🔒 Locked** for outlets or **🗑 Delete** for custom widgets (double-confirm: type the name). The expanded-row + cockpit-header action buttons (Deploy · Test · Configure · Disable) are a single **segmented pill bar** (one rounded container, `|` dividers, scrolls sideways on narrow screens — never wraps; 2026-06-16); the destructive **Delete** / static **🔒 Locked** sits outside the group as a red-tint / gray pill. A per-row **AUTH** dropdown (public/hmac/jwt) PATCHes instantly. Backend DELETE refuses outlet embeds (**403** if `store-*`/`bound_scope_id`) so an outlet can't be dropped even via API. Rows still **expand inline** → **KEYS** (embed_id · public_key + **↻ rotate** · **secret reveal** · endpoint), **CONFIG**, **drop-in snippet**, **FULL PHP CODE** (tabs `widget-embed.php` / `CityAgentClient.php`, HMAC secret stays a server-side `getenv()`). All confirmations use a branded in-app modal (`ConfirmModal.svelte`) — no native `confirm/prompt/alert`.
...
- **Access tab + flexible origins + 1-click allow (2026-06-15, plug-and-play Wave 1):** the widget cockpit has an **Access** tab (red count badge = pending sites). It sets the per-key **match mode** — `strict` (exact), `subdomains` (a listed `https://acme.com` also allows `https://shop.acme.com`), or `open` (any, insecure) — and edits the allow-list with **wildcards** (`https://*.acme.com`, `http://192.168.*`). Every **denied** Origin is auto-logged to `dash_embed_pending_origins` (migration 185) and listed under **Sites trying to embed** with **Allow** (appends to the key's allowlist instantly, no redeploy) / **Block**. This fixes the AWS *"Origin … not in allowlist (403)"* loop. Matcher lives in `dash/embed/session.py:verify_origin` (whole-string fnmatch — `https://*.acme.com` will NOT match `https://x.acme.com.evil.com`); per-key `origin_mode` column; CORS echo (`_per_embed_cors_headers`) uses the same matcher; `widget.js` shows a friendly *"ask the admin to allow this site"* message instead of a raw 403. Admin API: `GET /api/projects/{slug}/pending-origins`, `POST …/embeds/{id}/pending-origins/{allow,block}`. **The widget production proxy MUST forward the `Origin` header** — see `deploy/nginx/cityagent.conf` (`proxy_set_header Origin $http_origin`); a stripped Origin causes a false 403.
- **Versioned widget.js (Wave 2):** snippets append `?v=<version>` (from `/api/flags` `widget_version`) so customer browsers/CDNs pick up widget changes after each deploy. Deploy tab also shows **React/Next.js** + **WordPress** snippets. Full drop-in guide for embedding teams: `docs/EMBED.md`.
- **Claude-style widget UX (v1.46.0):** the embed chat now feels like Claude. The launcher is an **animated robot character** (no round bubble) — it bobs, blinks, pulses its antenna and waves. The header shows a **moody robot** that reacts to the conversation (idle → thinking → typing → done → error). The thinking trace has a **shimmering "Thinking" label**, no disconnected loading-dots (the answer streams in place under *"Thought for Ns"*), and a **blinking stream caret**. All motion respects `prefers-reduced-motion`. A `logo_url` on a key overrides the robot with the uploaded logo. Pure CSS/SVG in `dash/embed/widget.js` — no backend change.
- **Deployment map (v1.47.3):** the **Deployments** page has a `Manage widgets | Deployment map` toggle. The map answers *"one server, many sites"* — a table of every widget and the **website URL(s) it's live on** (one server serves all widgets; each widget can be embedded on one or many customer URLs). Add a destination site inline, remove with `×`, copy that widget's snippet per row, and see `live / ready / off` status at a glance.
- **App-robot, brand theme, bilingual + learned starters (v1.46.1 → v1.47.2):** the widget robot is now the **exact same character as the dashboard** (`FloatingRobot` — coral body, white eyes, smile, teal antenna) in the launcher, header and the empty-state hero. The whole widget is themed **coral to match the agent** (header, suggestion cards, send button). The empty state is **Claude-style cards** — a robot hero, a greeting, and tappable **suggested-question cards** (icon + chevron). **Bilingual replies**: ask in English → answer in English; ask in Burmese → answer in Burmese (the agent mirrors the question's language; numbers/codes stay unchanged). **Self-learning suggestions**: the suggested questions are pulled from the **real questions customers actually ask** (ranked by frequency from `dash_query_patterns`, generic/analyst queries filtered out), shuffled to **3 fresh ones each time the panel opens**; new installs fall back to a built-in bilingual pharma set until enough real usage accumulates. No LLM cost — served straight from the database. Endpoint: `GET /api/embed/config/{embed_id}/suggestions`.
...
- **Connector-credential encryption** — `CONNECTION_ENCRYPTION_KEY` (44-char urlsafe-b64 Fernet, secret ✅). **Auto-generated on first boot if unset (2026-06-14, Open WebUI `WEBUI_SECRET_KEY` pattern)** — persisted to `{KNOWLEDGE_DIR}/.connection_encryption_key` on the `knowledge_data` volume (survives recreate + rebuild) and reused on every restart, so **single-host installs need zero key config**. Resolution order: explicit `CONNECTION_ENCRYPTION_KEY` env → derive from `JWT_SECRET` → auto-created key file. **Set the explicit env for multi-host / multi-replica** (each container's auto-key is local, so hosts wouldn't share one). **Whichever key you end up on, keep it stable forever** — rotating it (or losing the knowledge volume) makes already-stored connector credentials undecryptable, so **back up the knowledge volume**. Generate an explicit one with `python -c "from cryptography.fernet import Fernet;print(Fernet.generate_key().decode())"`.
...
## Install (fresh box / AWS) — turnkey

### Plug-and-play (recommended): one command

```bash
git clone <repo-url> cityagent && cd cityagent
./install.sh                  # gen .env w/ random secrets, prompt model key, boot, print admin login
```

`./install.sh` runs preflight (docker/ports/RAM), generates `.env` with strong
random secrets (`scripts/bootstrap_env.sh`), prompts for `OPENROUTER_API_KEY`,
starts the stack, waits for health, and prints the **admin login + URL**. Safe to
re-run — it never overwrites an existing `.env`. Full guide: **[INSTALL.md](INSTALL.md)**.
...
The database schema seeds itself on first boot — **no manual SQL step**, **no
"relation … does not exist" wall** on a fresh DB.

```bash
git clone git@github.com:raahulgupta07/rahulai-pharma.git
cd rahulai-pharma
cp .env.example .env          # set DOMAIN (PUBLIC_URL auto-derives as https://<DOMAIN>) + OPENROUTER_API_KEY, DB_PASS, SUPER_ADMIN_PASS
...
frontend/src/lib/AgentFlow.svelte   animated cockpit flow diagram (counts = real DB values, auto-refresh every 30s; motion is cosmetic)
