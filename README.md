...
**Chat presentation — Claude-style (2026-06-13):** the analyst chat renders in a **`claude`** look by default — centered hero empty-state (`✻` greeting + composer-as-hero + subtle suggestion pills, no stat tiles), full-width answers (no avatar), a single `✻` "Thinking… → Thought for Xs" indicator, and a quiet gray collapsible reasoning trace. A **header toggle** (`✻ Claude` ⇄ `🤖 Classic`) flips back to the original robot-avatar look instantly (persisted in `localStorage`, no redeploy). The router-tier badge (`LOOKUP`/`ANALYSIS`) on each answer card sits inline next to the `⌖ source` chip.
