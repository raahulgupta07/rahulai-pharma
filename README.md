...
- **Admin console:** `/ui/embed` (super-admin) — **Overview · Widgets · Playground · Monitoring · Usage · Snippet & Docs**. (Config + Sandbox merged into **Playground** as of 2026-06-07; **Monitoring** added 2026-06-08.)
...
- **Disable surfaces (super-admin kill switch, 2026-06-08):** two toggles `gateway_enabled` / `embed_enabled` on **Admin → Cockpit → INTEGRATIONS card** (switches + **Save** — saving reloads so the nav updates) and also in Admin settings → Integrations. OFF = the surface vanishes from the Integrations nav **and** its API routes return 403 (`/api/v1*` for gateway, `/api/embed*` for embed) — enforced in `AuthMiddleware` (30s cached, fail-open). State surfaced via `/api/flags`, which reads the DB fresh (not the settings cache) so the nav reflects a change on the next reload.
...
- **Widgets admin list** (`#widgets`): rows **expand inline** (like Gateway Outlet Keys) → embed_id, public_key, endpoint, scope, rate, full snippet + copy, ▶ test + ⚙ playground. **↻ rotate** regenerates the public key (old one dies — store re-pastes snippet; `POST /embeds/{id}/rotate-key`) and **Revoke** disables the widget, both behind a confirm. One embed per store (`bound_scope_id=<site_code>`, `bound_intent=private`) = isolated per-store answers; bulk-mintable via `embed_mgr.create_embed`.
...
```bash
cp .env.example .env        # set OPENROUTER_API_KEY(S), DB_PASS, SUPER_ADMIN_PASS
docker compose up -d --build
```

### Fresh clone (AWS / new box)

```bash
git clone git@github.com:raahulgupta07/rahulai-city-pharma.git
cd rahulai-city-pharma
cp .env.example .env        # fill: real OpenRouter key, DB_PASS, admin pass
docker compose up -d --build
# poll http://127.0.0.1:8011/api/health → {"status":"ok"}
```

**Not in git** (gitignored — must be supplied/regenerated on the box):
...
- DB volume (`cp-db`) — fresh box = empty Postgres until data is uploaded.
