...
- **Disable surfaces (super-admin kill switch, 2026-06-08):** Admin → Admin settings → **Integrations** has two toggles `gateway_enabled` / `embed_enabled`. OFF = the surface vanishes from the Integrations nav **and** its API routes return 403 (`/api/v1*` for gateway, `/api/embed*` for embed) — enforced in `AuthMiddleware` (30s cached, fail-open), surfaced via `/api/flags`.
